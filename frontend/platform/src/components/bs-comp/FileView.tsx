...
const Row = React.memo(
  ({
    ...
    return (
      <div className="bg-[#fff] border-b-2 overflow-hidden" style={style}>
        {/* <span className="absolute">{index + 1}</span> */}
        {/* canvas  */}
        <div ref={wrapRef} className="canvasWrapper"></div>
        {/* label */}
        {labels && (
          <svg className="absolute top-0 w-full h-full z-30">
            {labels.map(
              (box) =>
                !areEqualFn(box.label) && (
                  <rect
                    key={box.id}
                    ...
                    onClick={() =>
                      onSelectLabel({ id: box.id, active: !box.active })
                    }
                  />
                ),
            )}
        ...
        <div
          ref={txtRef}
          className="textLayer absolute inset-0 overflow-hidden opacity-25 origin-top-left z-20 leading-none"
        ></div>
        {/* annotaions */}
        <div
          ref={annotRef}
          className="absolute inset-0 overflow-hidden origin-top-left z-20"
        ></div>
      </div>
    );
  },
...
export default function FileView({
    ...
    const throttledResizeHandler = throttle((entries) => {
      if (panneDom) {
        for (let entry of entries) {
          ...
          warpDom.style.setProperty(
            "--scale-factor",
            width / fileWidthRef.current + "",
          );
        }
      }
    }, 300);
  ...
  const hanleDragSelectLabel = useCallback(
    (start, end) => {
      let { x, y } = start;
      let { x: x1, y: y1 } = end;
      ...
      Object.keys(labels).forEach((key) => {
        ...
        pagelabels.forEach((item) => {
          ...
          const pageH =
            (key - startIndex) * ((boxSize.width / pageScale) * scale);
  ...
  const handleScroll = ({ scrollOffset }) => {
    ...
    onPageChange?.(
      scrollOffset,
      boxSize.height,
      boxSize.width / pageScale,
      fileWidthRef.current / boxSize.width,
    );
  ...
  const itemRenderer = useCallback(
    ...
    [pdf, drawfont, select, labels, boxSize],
  );

  return (
    <div
      ref={paneRef}
      className="flex-1 h-full bg-gray-100 rounded-md py-4 px-2 relative"
