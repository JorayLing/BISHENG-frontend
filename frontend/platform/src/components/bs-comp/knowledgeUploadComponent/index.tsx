...
const KnowledgeUploadComponent = ({
  ...
  useEffect(() => {
    ...
    const [uploadingList, awaitingList] = progressList.reduce(
      ([uploading, awaitList], pros) => {
    ...
    if (awaitingList.length === 0 && uploadingList.length === 0) {
      ...
      setProgressList((oldState) => {
        return oldState.map((pros) => {
          if (
            pros.progress === ProgressStatus.Await &&
            runCount++ < MaximumTask - uploadingList.length
          ) {
  ...
  const handleDrop = (acceptedFiles) => {
    ...
    const [bigFiles, files] = acceptedFiles.reduce(
      ([big, small], file) => {
        if (progressList.some((pros) => pros.fileName === file.name))
          return [big, small]; // 过滤重复文件
  ...
  const handleUploadResult = (id: string, result: any) => {
    ...
    // 临时去重方式
    successFilesRef.current = successFilesRef.current.filter(
      (pros) => pros.id !== id,
    );
  ...
  return (
    <div className="">
      ...
      <div
        className={cname("overflow-y-auto mt-4 space-y-2", progressClassName)}
      >
