...
export function TitleIconBg({ id, className = "", children = <SkillIcon /> }) {
  return (
    <div
      className={cname(
        `rounded-md flex justify-center items-center ${gradients[parseInt(id + "", 16) % gradients.length]}`,
        className,
      )}
    >
      {children}
...
export function TitleLogo({
  ...
  return url ? (
    <img
      src={url}
      className={cname(`w-6 h-6 rounded-sm object-cover`, className)}
    />
  ) : (
    <TitleIconBg id={id} className={className}>
      {children}
    </TitleIconBg>
  );
}
...
export default function CardComponent<T>({
  ...
  if (!id && type === "sheet")
    return (
      <Card
        className="group w-[320px] cursor-pointer border-dashed border-[#BEC6D6] transition hover:border-primary hover:shadow-none bg-background-new"
        onClick={onClick}
      >
        <CardHeader>
          <CardTitle>
            <div className="flex gap-2 items-center">
              <div className="justify-between">
                <PlusIcon className="group-hover:text-primary transition-none" />
              </div>
              <span>{title}</span>
            </div>
          </CardTitle>
        ...
        <CardFooter className="flex justify-end h-10">
          <div className="rounded cursor-pointer">
            <GoIcon className="group-hover:text-primary transition-none" />
          </div>
        </CardFooter>
      </Card>
    );
  ...
  if (!id)
    return (
      ...
      >
        ...
        <CardFooter className="flex justify-end h-10">
          <div className="rounded cursor-pointer">
            <GoIcon className="group-hover:text-primary transition-none dark:text-slate-50" />
          </div>
        </CardFooter>
      </Card>
    );
  ...
  if (type === "sheet")
    return (
      <Card
        className="group max-h-[146px] w-[316px] cursor-pointer bg-[#F7F9FC] dark:bg-background-main dark:hover:bg-background-login hover:bg-[#EDEFF6] hover:shadow-none relative"
        onClick={onClick}
      >
        <CardHeader className="pb-2">
          <CardTitle className="truncate-doubleline">
            <div className="flex gap-2 pb-2 items-center">
              <TitleLogo url={logo} id={id}>
                <Icon />
              </TitleLogo>
  ...
  return (
    <Card
      className="group w-[320px] hover:bg-card/80 cursor-pointer grid"
      ...
      <CardFooter className="h-20 grid grid-rows-2 self-end">
        ...
        <div className="flex justify-between items-center h-10">
          <div className="flex gap-1 items-center">
            ...
            <span className="text-sm font-medium leading-none overflow-hidden text-ellipsis max-w-32 ">
              {user}
            </span>
          </div>
          {edit && (
            <div className="hidden group-hover:flex">
              {!checked && (
                <div
                  className="hover:bg-[#EAEDF3] rounded cursor-pointer"
                  onClick={(e) => {
                    e.stopPropagation();
                    onSetting(data);
                  }}
