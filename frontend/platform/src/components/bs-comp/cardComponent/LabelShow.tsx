  ...
  const handleUpdate = (obj: { type: string; data: any }) => {
    switch (obj.type) {
      ...
      case UPDATETYPE.UPDATENAME: {
        setFreshData((pre) =>
          pre.map((d) =>
            d.value === obj.data.value ? { ...d, label: obj.data.label } : d,
          ),
        );
        onChange();
        break;
        ...
        isOperator ? (
          ...
          >
            <div
              ...
              className="mb-[10px] max-w-[100%] flex place-items-center rounded-sm p-1 border border-transparent group-hover:bg-search-input group-hover:border-input"
            >
