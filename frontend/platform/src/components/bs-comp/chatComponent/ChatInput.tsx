...
export default function ChatInput({
    ...
    return new Promise((res, rej) => {
      try {
        ...
        const ws = new WebSocket(
          `${webSocketProtocol}://${wsUrl}&chat_id=${chatId}`,
        );
        wsRef.current = ws;
        // websocket linsen
        ws.onopen = () => {
        ...
        ws.onmessage = (event) => {
          ...
          const errorMsg =
            data.category === "error" ? data.intermediate_steps : "";
          // 异常类型处理，提示
          ...
          // 群聊@自己时，开启input
          if (
            ["end", "end_cover"].includes(data.type) &&
            data.receiver?.is_self
          ) {
            setInputLock({ locked: false, reason: "" });
  ...
  const handleWsMessage = (data) => {
    ...
    if (data.type === "start") {
      ...
      if (
        msgClosedRef.current &&
        !["tool", "flow", "knowledge"].includes(data.category)
      ) {
      ...
      <div className={`relative ${clear && "pl-9"}`}>
        ...
        <div className="flex absolute left-0 top-4 z-10">
          {clear && (
            <div
              ...
              onClick={() => {
                !inputLock.locked && onClickClear?.();
              }}
            >
              <ClearIcon
                className={`${!showWhenLocked && inputLock.locked ? "text-muted-foreground" : "text-foreground"} dark:text-slate-50 dark:hover:bg-[#282828]`}
              ></ClearIcon>
            </div>
          )}
        </div>
        ...
        <div className="flex gap-2 absolute right-3 top-4 z-10">
          {stop.show ? (
            ...
            >
              <SendIcon
                className={`${inputLock.locked ? "text-muted-foreground" : "text-foreground"}`}
              />
            </div>
          )}
        </div>
        ...
        <Textarea
          ...
          placeholder={
            inputLock.locked ? inputLock.reason : t("chat.inputPlaceholder")
          }
          className={
            "resize-none py-4 pr-10 text-md min-h-6 max-h-[200px] scrollbar-hide dark:bg-[#2A2B2E] text-gray-800" +
            (form && " pl-10")
          }
          onKeyDown={(event) => {
