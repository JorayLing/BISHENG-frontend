...
export default function MessagePanne({
  debug = false,
  mark = false,
  logo,
  useName,
  ...
  return (
    <div
      id="message-panne"
      ref={messagesRef}
      className="h-full overflow-y-auto scrollbar-hide pt-12 pb-60"
    >
      {guideWord && (
        <MessageBs
          key={9999}
      ...
      {messagesList.map((msg, index) => {
        ...
        if (msg.isSend) {
          ...
          ["tool", "flow", "knowledge"].includes(msg.category)
        ...
        switch (type) {
          case "user":
            return (
              <MessageUser
                ...
                data={msg}
                onMarkClick={() =>
                  onMarkClick("question", msg.id, findQa(messagesList, index))
                }
              />
            );
          case "llm":
            return (
              <MessageBs
                ...
                onMarkClick={() =>
                  onMarkClick("answer", msg.id, findQa(messagesList, index))
                }
              />
            );
          case "system":
          ...
          case "reasoning":
            return (
              <ReasoningLog key={msg.id} loading={false} msg={msg.message} />
            );
          default:
            return (
              <div className="text-sm mt-2 border rounded-md p-2" key={msg.id}>
