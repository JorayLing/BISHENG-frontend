...
export default function ColFilterUser({ label, onFilter }) {
  ...
  const { options, setOptions, reload, loadMore, search } =
    useUsersOptions(label);
  ...
  return (
    <Popover open={open} onOpenChange={(bln) => setOpen(bln)}>
      <PopoverTrigger>
        <FilterIcon
          onClick={() => setOpen(!open)}
          className={value.length ? "text-primary ml-3" : "text-gray-400 ml-3"}
        />
      </PopoverTrigger>
      <PopoverContent>
        <div>
            ...
            {options.map((el) => (
              <div className="flex items-center space-x-2 mb-1" key={el.value}>
                <Checkbox
                  ...
                  onCheckedChange={() => handlerChecked(el.value)}
                />
                <Label
                  htmlFor={el.value}
...
const useUsersOptions = (label) => {
  ...
  const loadApps = () => {
    ...
    (label
      ? getLabelUsersApi(label)
      : getUsersApi({ page, pageSize: 400, name: keywordRef.current })
    ).then((res: any) => {
