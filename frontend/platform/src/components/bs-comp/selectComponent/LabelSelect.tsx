...
export default function LabelSelect({
  ...
  const handleChecked = (id) => {
    ...
    setData((pre) => {
      const newData = pre.map((d) =>
        d.value === id ? { ...d, selected: !d.selected } : d,
      );
      const cur = newData.find((d) => d.value === id);
      captureAndAlertRequestErrorHoc(
        (cur.selected
          ? createLinkApi(id, resource.id, type)
          : deleteLinkApi(id, resource.id, type)
        ).then(() => {
          onUpdate({
  ...
  const handleChange = (e, id) => {
    nameRef.current = id
      ? dataRef.current.find((d) => d.value === id).label
      : "";
    setData((pre) =>
      pre.map((d) => (d.value === id ? { ...d, label: e.target.value } : d)),
    );
  };

  const errorRestName = (preName, id) => {
    //错误发生回退初值
    preName
      ? setData((pre) =>
          pre.map((d) =>
            d.value === id ? { ...d, label: nameRef.current } : d,
          ),
        )
  ...
  const handleSave = async (e, id) => {
    if (e.key === "Enter") {
      setData((pre) =>
        pre.map((d) => (d.value === id ? { ...d, edit: false } : d)),
      );
      ...
      if (label.label.length > 10) {
        ...
        return message({
          title: t("prompt"),
          variant: "warning",
          description: t("tag.labelMaxLength"),
        });
      }
      const err = await captureAndAlertRequestErrorHoc(
        updateLabelApi(id, label.label).then((res: any) => {
          setData((pre) => {
            const newData = pre.map((d) =>
              d.value ? d : { ...d, label: res.name, value: res.id },
            );
            dataRef.current = newData;
            return newData;
          });
          ...
          return message({
            title: t("prompt"),
            variant: "success",
            description: id ? t("updateSuccess") : t("createSuccess"),
          });
        }),
      );
      if (!err) {
  ...
  const handleDelete = (label) => {
    bsConfirm({
      ...
      onOk(next) {
        captureAndAlertRequestErrorHoc(
          deleteLabelApi(label.value).then(() => {
            onUpdate({
  ...
  const handleSearch = (e) => {
    ...
    setKeyword(key);
    const newData = dataRef.current.filter((d) =>
      d.label.toUpperCase().includes(key.toUpperCase()),
    );
    setData(newData);
  };
  ...
  return (
    <Popover open={open} onOpenChange={handleOpenChange}>
      ...
      <PopoverContent className="z-[20]" onClick={(e) => e.stopPropagation()}>
        <div>
          <SearchInput
            placeholder={t("chat.searchLabels")}
            value={keyword}
        ...
        <div className="mt-4 h-[200px] overflow-y-auto">
          {data.map((d) => (
            <div className="flex group justify-between h-8 rounded-sm px-2 hover:bg-[#EBF0FF] dark:hover:bg-gray-700">
              <div className="flex place-items-center space-x-2">
                <Checkbox
                  id={d.value}
                  checked={d.selected}
                  onCheckedChange={() => handleChecked(d.value)}
                />
                {d.edit ? (
                  <Input
                    autoFocus
              ...
              {user.role === "admin" && (
                <div className="flex place-items-center gap-2 opacity-0 group-hover:opacity-100">
                  <SquarePen
                    className="size-4 cursor-pointer text-muted-foreground"
                    onClick={() => handleEdit(d.value)}
                  />
                  <TrashIcon
          ...
          {keyword && showAdd && user.role === "admin" && (
            <div
              onClick={handleAdd}
