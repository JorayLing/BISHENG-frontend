...
export default function SkillChatSheet({ children, onSelect }) {
  ...
  const loadData = (more = false) => {
    open &&
      getChatOnlineApi(pageRef.current, searchRef.current).then((res) => {
            ...
            {options.length ? (
              options.map((flow, i) => (
                <CardComponent
                  ...
                  footer={
                    <Badge
                      className={`absolute right-0 bottom-0 rounded-none rounded-br-md ${flow.flow_type === "flow" && "bg-gray-950"}`}
                    >
                      {flow.flow_type === "flow"
                        ? t("build.skill")
                        : t("build.assistant")}
