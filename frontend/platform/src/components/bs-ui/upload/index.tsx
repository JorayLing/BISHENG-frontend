  ...
  (
    ...
    const onDrop = (acceptedFiles) => {
      ...
      acceptedFiles.forEach((file) => {
        file.size < sizeLimit ? files.push(file) : errorFile.push(file.name);
      });
      errorFile.length &&
        message({
          title: t("prompt"),
          description: errorFile.map(
            (str) =>
              `${t("code.file")}: ${str} ${t("code.sizeExceedsLimit", { size })}`,
          ),
        });
    ...
    useEffect(() => {
      ...
      if (peddings.length || awaits.length) {
        ...
        // 任务补位
        awaits
          .filter((e, i) => i < requestCount - peddings.length)
          .forEach((task) => {
            // task为补位任务
            // 标记开始上传
            ...
            uploadFileWithProgress({
              ...
              if (data) {
                filePathsRef.current.push({
                  id: task.id,
                  name: task.file.name,
                  path: data.file_path,
                });
                onFileCountChange(
                  filePathsRef.current.length,
                  progressCountRef.current,
                );
              } else {
                failFilesRef.current.push({
                  id: task.id,
                  name: task.file.name,
                });
                setProgressList((oldState) =>
    ...
    return (
      <div>
        <div
          {...getRootProps()}
          className="group h-[100px] border border-dashed rounded-md flex flex-col justify-center items-center cursor-pointer gap-3 hover:border-primary"
        >
          <input {...getInputProps()} />
          <UploadIcon className="group-hover:text-primary" />
          {isDragActive ? (
            <p className="text-gray-400 text-sm">{t("code.dropFileHere")}</p>
          ) : (
            <p className="text-gray-400 text-sm">{t("code.clickOrDragHere")}</p>
          )}
        </div>
        <div
          className={cname(
            "overflow-y-auto no-scrollbar mt-4",
            progressClassName,
          )}
        >
          {progressList.map((pros) => (
            <div key={pros.id}>
              <p
                className={`max-w-[300px] overflow-hidden text-ellipsis whitespace-nowrap ${pros.error && "text-red-400"}`}
              >
                {pros.file.name}
                {pros.file.pros === 1 && <span>{t("code.complete")}</span>}
              </p>
              <div className="w-full flex">
                <Progress error={pros.error} value={pros.pros} />
                <X
                  className="hover:bg-gray-200 ml-2 cursor-pointer"
                  onClick={() => {
                    setProgressList((oldState) =>
                      oldState.filter((el) => el.id !== pros.id),
                    );
                    filePathsRef.current = filePathsRef.current.filter(
                      (el) => el.id !== pros.id,
                    );
                    failFilesRef.current = failFilesRef.current.filter(
                      (el) => el.id !== pros.id,
                    );
                    onFileCountChange(
                      filePathsRef.current.length,
