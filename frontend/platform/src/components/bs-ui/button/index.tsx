  ...
  "inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        ...
        black:
          "bg-black-button text-primary-foreground shadow hover:bg-black-button/85",
      },
      size: {
...
const ButtonNumber = React.forwardRef<
  HTMLButtonElement,
  {
    className?: string;
    defaultValue?: number;
...
>(
  (
    ...
    return (
      <div
        className={cname(
          "flex items-center border input-border bg-gray-50 dark:bg-background-login rounded-md",
          className,
        )}
      >
        <Button
...
>(({ loading = false, disabled = false, ...props }, ref) => {
  return (
    <Button {...props} disabled={loading || disabled} ref={ref}>
      {loading && <LoadIcon className="mr-1" />}
      {props.children}
    </Button>
  );
});

export { Button, ButtonNumber, buttonVariants, LoadButton };
