...
const MultiItem: React.FC<{
  ...
  return (
    <div
      key={value}
      className={`relative flex justify-center py-[5px] select-text-font cursor-default select-none items-center rounded-sm  mb-1 text-sm outline-none hover:bg-[#EBF0FF] dark:hover:bg-gray-700 hover:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50 break-all 
    ${active && "bg-[#EBF0FF] dark:bg-gray-700 text-[#0057FF]"}`}
      onClick={() => {
        onClick(value, children as string);
      }}
    >
      <span className="absolute right-2 flex h-3.5 w-3.5 items-center justify-center">
        {active && <Check color={"#5A87FB"} className="h-4 w-4"></Check>}
      </span>
      {children}
    </div>
  );
...
const MultiSelect = ({
  ...
  const handleSwitch = (value: string, label: string) => {
    ...
    if (onScrollLoad) {
      const newValues = (values as Option[]).some(
  ...
  return (
    ...
    >
      <SelectTrigger
        className={cname(
          `group  py-1 ${error && "border-red-500"} ${scroll ? "h-9 overflow-y-auto items-start pt-1.5" : "h-auto"}`,
          className,
        )}
        ref={triggerRef}
      >
        {!multiple &&
          (values.length ? (
            <span className="text-foreground">
              {onScrollLoad
                ? (values[0] as Option).label
                : options.find((op) => op.value === values[0])?.label}
            </span>
          ) : (
            placeholder
          ))}
            ...
            onScrollLoad ? (
              <div className="flex flex-wrap w-full">
                {values.map((item) => (
                  ...
                  >
                    {item.label}
                    {lockedValues.includes(item.value) || (
                      <X
                  ...
                  options
                    ...
                    .map((option) => (
                      <Badge
                        onPointerDown={(e) => e.stopPropagation()}
                        key={option.value}
                        className="flex whitespace-normal items-center gap-1 select-none bg-primary/20 text-primary hover:bg-primary/15 m-[2px] break-all"
                      >
                        {option.label}
                        {lockedValues.includes(option.value) || (
                          <X
                            className="h-3 w-3 min-w-3"
      ...
      >
        <div>
          {optionFilter.map((item) => (
            <MultiItem
              active={values.some(
                (val) => val === item.value || val.value === item.value,
              )}
              value={item.value}
