...
export default function Avator({
  ...
  const handleFileChange = (event) => {
    ...
    if (file) {
      ...
      const errormgs = [];
      if (!isValidSize)
        errormgs.push(`${t("build.fileSizeLimit")} ${size / 1024 / 1024}MB`);
  ...
  return (
    <div className={cname("flex w-full rounded-md gap-4", className)}>
      <div className="relative group">
        {value ? (
          <img
            src={value}
            className="max-w-24 max-h-24 min-w-8 min-h-8 rounded-md object-cover border"
            alt=""
          />
        ) : (
          children
        ...
        <div className="absolute left-0 top-0 w-full h-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black/50 flex items-center justify-center rounded-md">
          ...
          {close && value && (
            <div
              className="absolute -top-3 -right-3 bg-gray-50 rounded-full p-1 cursor-pointer border border-gray-300"
