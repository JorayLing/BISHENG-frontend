...
const Item = (props: {
  ...
  const handleClick = () => {
    const _isAsync =
      isAsync && !(option.children && option.children.length !== 0); // 需要异步加载
  ...
  return (
    ...
    >
      ...
      {!isLeaf &&
        (loading ? (
          <LoadIcon className="text-foreground" />
        ) : (
...
const Col = (props: {
  ...
  return (
    <div className="w-36 border-l first:border-none max-h-80 overflow-y-auto">
      {options.map((option) => (
        <Item
          {...opros}
          option={option}
...
export default function Cascader({
  ...
  const handleHover = (option, isLeaf, colIndex) => {
    ...
    // // setValues([]) // 从新选择清空值
    const isAsync =
      loadData && !(option.children && option.children.length !== 0); // 需要异步加载
    if (!(isAsync || isLeaf)) {
      setCols((cols) => {
  ...
  const handleClick = (option, isLeaf) => {
    if (!isLeaf) {
      const isAsync =
        loadData && !(option.children && option.children.length !== 0);
      if (!isAsync) return;
  ...
  return (
    <Select open={open} onOpenChange={setOpen}>
      <SelectTrigger
        className={`${error && "border-red-500"} group data-[placeholder]:text-inherit ${selectClass}`}
      >
        <Input
      ...
      <SelectContent auto>
        {cols.length ? (
          ...
          <div className="w-full flex justify-center items-center bisheng-label">
            空
          </div>
        )}
      </SelectContent>
