...
export const TableHeadEnumFilter = ({
  options,
  onChange,
}: {
    ...
    >
      <SelectPrimitive.Trigger className="outline-none">
        <FilterIcon
          ...
          className={value ? "text-primary ml-3" : "text-gray-400 ml-3"}
        />
  ...
  return (
    <div className="h-full">
      ...
      <div className="mt-2 max-h-[260px] min-h-20 overflow-y-auto">
        {options.map((i) => (
          <div
            className="flex items-center space-x-2 text-gray-500 mb-1"
            key={i.id}
          >
            <Checkbox
              id={i.id}
              checked={value.includes(i.id)}
              onCheckedChange={() => onChecked(i.id)}
            />
            <label htmlFor={i.id} className="cursor-pointer text-sm">
              {i[nameKey]}
            </label>
          </div>
        ))}
      ...
      <div className="flex justify-between mt-4">
        <Button variant="ghost" className="px-8 h-8" onClick={onClearChecked}>
          {t("system.reset")}
        </Button>
        <Button className="px-8 h-8" onClick={onOk}>
          {t("system.confirm")}
        </Button>
      </div>
    </div>
