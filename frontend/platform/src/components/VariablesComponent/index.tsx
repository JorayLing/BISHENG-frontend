...
export default function VariablesComponent({
  ...
  const handleSave = async (_item) => {
    ...
    const param: any = {
      ...
      value:
        _item.type === VariableType.Text
          ? _item.maxLength
          : _item.options.map((el) => el.value).join(","),
    };
    if (_item.update) {
      param.id = _item.id;
    }
    captureAndAlertRequestErrorHoc(
      saveVariableApi(param).then((res) => {
        const _items = items.map((item) =>
          item.id === _item.id ? { ..._item, id: res.id, update: true } : item,
        );
  ...
  const handleDelClick = async (index) => {
    ...
    item[0].update &&
      (await captureAndAlertRequestErrorHoc(delVariableApi(item[0].id)));
    setItems(newItems);
    // 触发必填校验
      ...
      {items.map((item, idx) => {
        return (
          <div key={idx} className="flex w-full gap-3">
            <div
              className="input-primary min-h-8"
