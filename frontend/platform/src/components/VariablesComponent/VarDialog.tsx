...
export default function VarDialog({ data, onSave, onClose }) {
  ...
  return (
    <DialogContent className="sm:max-w-[425px]">
      ...
      >
        <TabsList className="">
          <TabsTrigger value="text" className="roundedrounded-xl">
            {t("flow.text")}
          </TabsTrigger>
        ...
        <TabsContent
          value="select"
          className="pb-10 px-2 max-h-80 overflow-y-auto scrollbar-hide"
        >
          {VariablesName}
              ...
              {(provide) => (
                <div {...provide.droppableProps} ref={provide.innerRef}>
                  {item.options.map((option, index) => (
                    <Draggable
                      key={"li" + option.key}
                      draggableId={"li" + option.key}
                      index={index}
                    >
                      {(provided, snapshot) => (
                        <div
                          className="flex mt-2 gap-2 select-none"
                        ...
                        >
                          <Input
                            value={option.value}
                            className={errors[index] && "border-red-400"}
                            onChange={(e) =>
      ...
      <DialogFooter>
        ...
        <Button className="px-8" onClick={handleSave} size="sm">
          {t("save")}
        </Button>
      </DialogFooter>
      <div className="flex mt-4 justify-end gap-4"></div>
    </DialogContent>
  );
}
