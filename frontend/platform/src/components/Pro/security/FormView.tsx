  ...
  return (
    <div className="mb-4 px-6">
      <div className="flex items-center mb-4">
        ...
        <span className="bg-gray-200 dark:bg-slate-900 px-2 rounded-xl text-xs">
          {t("build.sensitiveWordMatch")}
        </span>
      </div>
      <div className="flex items-center mb-4">
        <span className="bisheng-label">{t("build.wordListType")}：</span>
        <div className="inline">
          {data.wordsType?.map((v, index) => (
            <span
              key={index}
              className="mr-2 bg-gray-200 dark:bg-slate-900 px-2 rounded-xl text-xs"
            >
              {map[v]}
            </span>
          ))}
