...
export default function FlowSetting({ id, type, isOnline, onSubTask }) {
  ...
  const handleFormChange = async (_form) => {
    ...
    const callBack = async (id) => {
      ...
      message({
        title: t("prompt"),
        variant: "success",
        description: t("build.saveSuccess"),
      });
    };
  ...
  return (
    <div>
      <div className="mt-6 flex items-center h-[30px] mb-4 px-6">
        ...
        <div className="flex items-center ml-6">
          <Sheet open={open} onOpenChange={(bln) => setOpen(bln)}>
            ...
            <SheetContent
              className="w-[500px] bg-background-login"
              onClick={(e) => e.stopPropagation()}
            >
              <SheetTitle className="font-[500] pl-3 pt-2">
      ...
      <div className="text-sm">{form.isCheck && <FormView data={form} />}</div>
    </div>
  );
}
