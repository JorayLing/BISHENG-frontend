...
export default function FormSet({ data, onChange, onSave, onCancel }) {
  ...
  const handleCheckboxChange = (val, name) => {
    ...
    val
      ...
      : setForm({
          ...form,
          wordsType: temp.filter((v) => v !== name),
        });
  };
  ...
  return (
    <>
      <div className="px-4 mt-6">
        ...
        <RadioGroup value="1" className="flex space-x-2 h-[20px] mt-4 mb-6">
          <div>
            <Label className="flex justify-center">
              <RadioGroupItem className="mr-2" value="1" />
              {t("build.sensitiveWordListMatch")}
            </Label>
          </div>
          <div>
            <Label className="flex justify-center">
              <RadioGroupItem disabled className="mr-2" value="模型审查" />
              {t("build.modelReview")}
            </Label>
          </div>
        </RadioGroup>
