...
export default function HeaderMenu({}) {
  ...
  if (
    ["/build/apps", "/build/tools", "/build/client"].includes(location.pathname)
  ) {
    return (
      <div className="build-tab flex justify-center h-[65px] items-center relative">
        {/* <div className="px-4">
        ...
        <div className="px-4">
          <NavLink
            to={"build/apps"}
            className="group flex gap-2 items-center px-8 py-2 rounded-md navlink"
          >
            <TabIcon className="text-muted-foreground group-hover:text-primary dark:group-hover:text-[#fff]"></TabIcon>
            <span className="text-sm font-bold text-muted-foreground group-hover:text-primary dark:group-hover:text-[#fff]">
              {t("build.app")}
            </span>
          </NavLink>
        </div>
