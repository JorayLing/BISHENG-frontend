...
export async function getLogsApi({
  ...
  const uids =
    userIds?.reduce((pre, val) => `${pre}&operator_ids=${val}`, "") || "";
...
export async function getModulesApi(): Promise<{ data: any[] }> {
  return {
    data: [
      { name: "log.chat", value: "chat" },
      { name: "log.build", value: "build" },
      { name: "log.knowledge", value: "knowledge" },
      { name: "log.system", value: "system" },
    ],
  };
}
...
export async function getActionsByModuleApi(moduleId) {
  switch (moduleId) {
    case "chat":
      return actions.filter((a) => a.value.includes("chat"));
    case "build":
      return actions.filter((a) => a.value.includes("build"));
    case "knowledge":
      return actions.filter(
        (a) => a.value.includes("knowledge") || a.value.includes("file"),
      );
    case "system":
      return actions.filter(
        (a) => a.value.includes("user") || a.value.includes("role"),
      );
  }
}
...
// 创建标注任务
export async function createMarkApi(data: {
  app_list: string[];
  user_list: string[];
}) {
...
// 标注会话列表
export async function getMarkChatsApi({
  task_id,
  keyword,
  page,
  pageSize,
...
// 更新标注状态
export async function updateMarkStatusApi(data: {
  session_id: string;
  task_id: number;
  status: number;
}) {
