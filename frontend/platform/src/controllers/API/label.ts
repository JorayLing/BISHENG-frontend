...
export async function getAllLabelsApi() {
  return await axios.get("/hjapi/v1/tag");
}

// admin全局创建一个标签
export async function createLabelApi(name: string) {
  return await axios.post("/hjapi/v1/tag", {
    name,
  });
}
...
export async function updateLabelApi(id: number, name: string) {
  return await axios.put("/hjapi/v1/tag", {
    tag_id: id,
    name,
  });
}
...
export async function deleteLabelApi(id: number) {
  return await axios.delete("/hjapi/v1/tag", {
    data: {
      tag_id: id,
    },
  });
...
export async function createLinkApi(
  ...
  return await axios.post("/hjapi/v1/tag/link", {
    tag_id,
    resource_id,
    resource_type,
  });
...
export async function deleteLinkApi(
  ...
  return await axios.delete("/hjapi/v1/tag/link", {
    data: {
      tag_id,
      resource_id,
      resource_type,
...
export async function getHomeLabelApi() {
  return await axios.get("/hjapi/v1/tag/home");
}

// 更新首页展示的标签列表
export async function updateHomeLabelApi(tag_ids) {
  return await axios.post("/hjapi/v1/tag/home", {
    tag_ids,
  });
}
