...
export const Finetune = () => {
  ...
  return (
    <div className="relative bg-background-main-content pt-2">
                  ...
                  <TableBody>
                    {tasks.map((task: any) => (
                      <TableRow
                        ...
                        className={`cursor-pointer ${task.id === taskId && "bg-gray-100"}`}
                      >
                        <TableCell className="font-medium">
                          {task.model_name}
                        </TableCell>
                        <TableCell>
                          <BadgeView value={task.status}></BadgeView>
                        </TableCell>
                        <TableCell>{task.server_name}</TableCell>
                        <TableCell className="text-right">
                          {task.create_time.replace("T", " ")}
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
      ...
      <Dialog
        open={showCpu.show}
        onOpenChange={(show) => setShowCpu({ ...showCpu, show })}
      >
