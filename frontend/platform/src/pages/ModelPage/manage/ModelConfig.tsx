...
function ModelItem({ data, type, onDelete, onInput, onConfig }) {
  ...
  const [isWebSearchEnabled, setIsWebSearchEnabled] = useState(
    data.config?.enable_web_search || false,
  );
    ...
    <div className="group w-full border rounded-sm p-4 mb-2">
      ...
      <div className="space-y-2 mt-2">
        ...
        {model.model_type === "llm" && (
          <>
            {["qwen", "tencent", "moonshot"].includes(type) && (
...
export default function ModelConfig({
  ...
  const handleTypeChange = (val) => {
    const name = onGetName(
      _modelProvider.find((el) => el.value === val).name || "",
    );
    // 自动填充模型
  ...
  const handleAddModel = () => {
    // 找最大
    const maxIndex = formData.models.reduce(
      (max, el, i) =>
        el.name.match(/model (\d+)/)
          ? Math.max(max, +el.name.match(/model (\d+)/)[1])
          : max,
      0,
    );
  ...
  const handleModelChange = (name, type, index) => {
    const models = formData.models.map((el, i) =>
      index === i
        ? {
            ...el,
    ...
    setFormData({ ...formData, models });
    // 重复校验
    return formData.models.find(
      (el, i) => index !== i && el.model_name === name,
    );
  };
    ...
    try {
      ...
      if (id === -1) {
        await captureAndAlertRequestErrorHoc(
          addLLmServer({ ...formData, config }).then((res) => {
            // if (res.code === 10802) {
            //     return toast({
            //         variant: 'error',
            //         description: res.msg
        ...
        await captureAndAlertRequestErrorHoc(
          updateLLmServer({ ...formData, config }).then((res) => {
  ...
  return (
    <div className="relative size-full py-4">
      <div className="flex ml-6 items-center gap-x-3">
        ...
        <span>
          {id === -1 ? t("model.addModel") : t("model.modelConfiguration")}
        </span>
      </div>
      <div className="w-[50%] min-w-64 px-4 pb-10 mx-auto mt-6 h-[calc(100vh-220px)] overflow-y-auto">
        ...
        <div className="mb-2">
          <Label className="bisheng-label">
            ...
            <QuestionTooltip
              className="relative top-0.5 ml-1"
              content={t("model.serviceProviderNameTooltip")}
            />
          </Label>
          <Input
            value={formData.name}
            onChange={(e) => {
              ...
              document.getElementById(
                "model_provider_name_error",
              ).style.display = !name || name.length > 100 ? "block" : "none";
            }}
          ></Input>
          <span
            id="model_provider_name_error"
            style={{ display: "none" }}
        ...
        <div className={formData.type ? "visible" : "invisible"}>
          <div className="mb-2">
            <div className="flex items-center gap-x-6">
              ...
              <Switch
                checked={formData.limit_flag}
                onCheckedChange={(val) =>
                  setFormData((form) => ({ ...form, limit_flag: val }))
                }
              />
              <div
                className={`flex items-center gap-x-2 ${formData.limit_flag ? "" : "invisible"}`}
              >
                <Input
                  type="number"
          ...
          <div className="mb-2">
            ...
            <div className="w-[92%]">
              ...
              <Button
                className="w-full mt-2 border-dashed border-border"
                variant="outline"
      ...
      <div className="absolute right-0 bottom-0 p-4 flex gap-4">
        {id !== -1 && (
          <Button
            className="px-8"
            variant="destructive"
            onClick={handleModelDel}
          >
            {t("model.delete")}
          </Button>
        )}
        <Button className="px-8" variant="outline" onClick={() => onBack()}>
          {t("model.cancel")}
        </Button>
        <LoadButton
          className="px-16"
...
const useSelectModel = () => {
  ...
  useEffect(() => {
    loadData().then((res) => (modelsRef.current = res));
  }, []);

  return (type) => {
    return (modelsRef.current?.[type] || []).map((item) => ({
      ...item,
      id: generateUUID(4),
    }));
  };
