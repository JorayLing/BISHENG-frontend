...
const ModelRow = ({ item, index, llmOptions, updateField, deleteRow }) => {
  ...
  return (
    <div
      className="grid mb-4 items-center"
      style={{
        gridTemplateColumns: "repeat(2, 1fr) 80px 110px 76px 90px 40px",
      }}
    >
      <div className="pr-2">
        <ModelSelect
      ...
      <div className="pr-2">
        <Select
          value={item.agent_executor_type}
          onValueChange={(val) =>
            updateField(index, "agent_executor_type", val)
          }
        >
          <SelectTrigger>
      ...
      <div className="pr-2">
        <Select
          value={item.streaming ? "1" : "0"}
          onValueChange={(val) => updateField(index, "streaming", val === "1")}
        >
          <SelectTrigger>
            <SelectValue placeholder="" />
          </SelectTrigger>
        ...
        <Input
          ...
          onChange={(e) =>
            updateField(
              ...
              Math.min(parseInt(e.target.value), 15000),
            )
          }
          min={1}
          max={15000}
      ...
      <div className="pr-2">
        <Select
          value={item.knowledge_sort_index ? "1" : "0"}
          onValueChange={(val) =>
            updateField(index, "knowledge_sort_index", val === "1")
          }
        >
          <SelectTrigger>
            <SelectValue placeholder="" />
      ...
      <div className="m-auto">
        <RadioGroup
          ...
          onValueChange={(val) => updateField(index, "default", val === "1")}
        >
...
export default function AssisModel({ llmOptions, onBack }) {
  ...
  const deleteRow = (index) => {
    let target = null;
    const updatedList = form.llm_list
      .filter((_, i) => {
        if (i === index) {
          target = _;
          return false;
  ...
  const handleSave = () => {
    ...
    if (form.llm_list.some((el) => el.model_id === null)) {
      return message({
        variant: "error",
        description: t("model.assistantInferenceModel") + t("bs:required"),
      });
    }
    if (form.auto_llm.model_id === null) {
      return message({
        variant: "error",
        description:
          t("model.assistantAutoOptimizationModel") + t("bs:required"),
      });
    }
    ...
    if (uniqueList.length !== form.llm_list.length) {
      return message({
        variant: "error",
        description: t("model.assistantInferenceModelRepetition"),
      });
    }

    captureAndAlertRequestErrorHoc(
      updateAssistantModelConfig(form).then((res) => {
  ...
  return (
    <div className="w-[70vw] mx-auto pt-2">
      <div className="mb-6">
        ...
        <div className="mt-2 border p-4 rounded-md bg-muted">
          <div
            className="grid mb-4 items-center"
            style={{
              gridTemplateColumns: "repeat(2, 1fr) 80px 110px 68px 90px 40px",
            }}
          >
            ...
            <Label className="bisheng-label">
              ...
              <QuestionTooltip
                ...
                content={t("model.assistantExecutionModeTooltip")}
              />
            </Label>
            <Label className="bisheng-label">
              {t("model.streamingOutput")}
            </Label>
            <Label className="bisheng-label">
              <span>{t("model.assistantKnowledgeBaseMaxCharacters")}</span>
              <QuestionTooltip
                className="relative top-0.5 ml-1"
                content={t("model.assistantKnowledgeBaseMaxCharactersTooltip")}
              />
            </Label>
            <Label className="bisheng-label">
              ...
              <QuestionTooltip
                className="relative top-0.5 ml-1"
                content={t("model.reorderAfterRetrievalTooltip")}
              />
            </Label>
            <Label className="bisheng-label text-center">
      ...
      <div className="">
        <span className="pl-1">
          {t("model.assistantAutoOptimizationModel")}
        </span>
        <div className="mt-2 border p-4 rounded-md bg-muted">
          ...
          <div className="grid grid-cols-4 gap-2 mt-4">
            ...
            />
            <Select
              value={form.auto_llm.streaming ? "1" : "0"}
              onValueChange={(val) =>
                updateAutoLLMField("streaming", val === "1")
              }
            >
              <SelectTrigger>
                <SelectValue placeholder="" />
