...
export default function EvaluationModel({ llmOptions, onBack }) {
  ...
  const handleSave = () => {
    if (!selectedModel) {
      return message({
        variant: "error",
        description: t("model.defaultEvaluationFeature") + t("bs:required"),
      });
    }
    const data = {
    ...
    captureAndAlertRequestErrorHoc(
      updateEvaluationModelConfig(data).then((res) => {
    ...
    <div className="max-w-[520px] mx-auto">
      <div className="mt-10">
        <Label className="bisheng-label">
          {t("model.defaultEvaluationFeature")}
