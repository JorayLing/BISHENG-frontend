...
export default function KnowledgeModel({ llmOptions, embeddings, onBack }) {
  ...
  const handleSave = async () => {
    ...
    if (errors.length)
      return message({ variant: "error", description: errors });
    ...
    await captureAndAlertRequestErrorHoc(
      updateKnowledgeModelConfig(data).then((res) => {
  ...
  return (
    <div className="max-w-[520px] mx-auto gap-y-4 flex flex-col mt-16 relative">
      ...
      <div className="absolute top-44 -right-28">
        <PromptDialog
          ...
          onRestore={() =>
            setForm({
              ...form,
              abstractPrompt: lastSaveFormDataRef.current.abstract_prompt,
            })
          }
        >
          <Tip
            content={"文档知识库上传文件时，对文件进行摘要的提示词"}
            side={"top"}
          >
            <Button variant="link">
              <Settings size={14} className="mr-1" /> 编辑提示词
            </Button>
          </Tip>
        </PromptDialog>
      </div>
      <div className="mt-10 text-center space-x-6">
        ...
        <Button className="px-10" disabled={saveload} onClick={handleSave}>
