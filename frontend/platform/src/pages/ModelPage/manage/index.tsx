...
function CustomTableRow({ data, index, user, onModel, onCheck }) {
  ...
  return (
    <div className="text-sm bs-table-row">
      <div
        className={`grid grid-cols-2 transition-colors hover:bg-muted/50 items-center mt-1 mx-2 h-[52px] rounded-sm`}
      >
        <div className="bs-table-td h-full p-2 flex items-center gap-x-3 first:rounded-l-md last:rounded-r-md font-medium">
          {expand ? (
            <CircleMinus
              className="cursor-pointer min-w-4 w-4 h-4"
      ...
      {expand && (
        <div className="px-12 py-2 m-auto border-collapse">
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="w-[200px] min-w-[100px]">
                  {t("model.modelType")}
                </TableHead>
                <TableHead className="w-[200px] min-w-[100px]">
                  {t("model.status")}
                ...
                <TableHead className="w-[100px] min-w-[100px]">
                  {t("model.onlineOfflineOperation")}
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {data.models.map((m) => (
                <TableRow key={m.id}>
                  ...
                  <TableCell>
                    <span
                      className={
                        ["text-green-500", "text-orange-500", "text-gray-500"][
                          m.status
                        ]
                      }
                    >
...
export default function Management() {
  ...
  const handleCheck = (index, bool, id) => {
    captureAndAlertRequestErrorHoc(
      changeLLmServerStatus(id, bool).then((res) => {
        refetch();
      }),
    );
    data[index].models = data[index].models.map((el) =>
      el.id === id ? { ...el, online: bool } : el,
    );
    setData([...data]);
  };
  ...
  if (modelId)
    return (
      <ModelConfig
        ...
        onReload={reload}
        onBerforSave={(id, name) =>
          data.some((el) => el.name === name && el.id !== id)
        }
        onAfterSave={(msg) => {
  ...
  if (systemModel)
    return (
      <SystemModelConfig data={data} onBack={() => setSystemModel(false)} />
    );
    ...
    <div className="relative bg-background-login h-full px-2 py-4">
      ...
      <div className="h-full overflow-y-auto">
        <div className="flex justify-end gap-4">
          ...
          {user.role === "admin" && (
            <Button onClick={() => setModelId(-1)}>
              {t("model.addModel")}
            </Button>
          )}
          <Button className="bg-black-button" onClick={reload}>
            {t("model.refresh")}
          </Button>
        </div>
        <div className="h-[85%]">
