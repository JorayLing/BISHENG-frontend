...
export const BadgeView = ({ value }) => {
  ...
  const texts = [
    "",
    t("finetune.trainingInProgress"),
    t("finetune.trainingFailed"),
    t("finetune.taskAborted"),
    t("finetune.trainingSuccess"),
    t("finetune.publishSuccess"),
  ];
  return (
    <Badge
      size="sm"
      variant="secondary"
      className={`${colors[value]} break-keep`}
    >
      {texts[value]}
    </Badge>
  );
    ...
    <div>
      ...
      <div className="border-b pb-4">
        <div className="flex gap-4 mt-4">
          ...
          <small className="text-sm font-medium leading-none text-gray-700">
            {baseInfo.create_time.replace("T", " ")}
          </small>
        </div>
        <div className="flex gap-4 mt-4">
          <small className="text-sm font-medium leading-none text-gray-500">
          ...
          <small className="text-sm font-medium leading-none text-gray-700">
            {TaskStatus.TRAINING_IN_PROGRESS === baseInfo.status
              ? "--"
              : formatMilliseconds(
                  baseInfo.report?.train_runtime || 0,
                  t("finetune.formatTime"),
                )}
          </small>
        </div>
        <div className="flex gap-4 mt-4">
      ...
      <div className="border-b pb-4">
        <div className="flex gap-4 mt-4">
          ...
          <small className="text-sm font-medium leading-none text-gray-700">
            {baseInfo.preset_data
              .concat(baseInfo.train_data)
              .map((el) => el.name)
              .join(",")}
          </small>
        </div>
