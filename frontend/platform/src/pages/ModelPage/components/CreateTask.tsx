...
import {
  RadioGroup,
  RadioGroupItem,
} from "../../../components/bs-ui/radio-group";
import {
  Table,
  TableBody,
  TableCell,
...
export default function CreateTask({ rtClick, gpuClick, onCancel, onCreate }) {
  ...
  const defaultTable = [
    ...
    { name: "val_ratio", value: "0.1", desc: t("finetune.valRatioDesc") },
    {
      ...
      value: "1",
      desc: t("finetune.batchSizeDesc"),
    },
    {
      name: "learning_rate",
      value: "0.00005",
      desc: t("finetune.learningRateDesc"),
    },
  ...
  const resultRef = useRef({
    ...
    base_model: "",
    model_name: "",
    extra_params: defaultTable.reduce((res, el) => {
  ...
  const handleCreate = async () => {
    ...
    if (!resultRef.current.base_model)
      errors.push(t("finetune.selectBaseModel"));
    if (
      !/^(?=.*[a-zA-Z])(?=.*\d)?[a-zA-Z\d_-]+$/.test(
        resultRef.current.model_name,
      )
    )
      errors.push(t("finetune.enterModelName"));
    ...
    const res = await captureAndAlertRequestErrorHoc(
      createTaskApi(resultRef.current),
    );
    setLoading(false);
    if (!res) return;
  ...
  return (
    <div className="pt-2 h-[calc(100vh-162px)] px-2 overflow-y-auto">
      <div className="border-b pb-2 flex justify-between items-center">
        <h1 className="">{t("finetune.createTrainingTask")}</h1>
        {/* <Button variant="black" onClick={rtClick}>FT服务管理</Button> */}
        <Button variant="black" onClick={rtClick}>
          {t("finetune.rtServiceManagement")}
        </Button>
      </div>
      {/* base */}
      <div className="border-b pb-4">
        <div className="flex gap-4 flex-col mt-4">
          ...
          <div className="flex gap-4 items-center">
            ...
            <Button size="sm" onClick={gpuClick}>
              {t("finetune.gpuResourceUsage")}
            </Button>
          </div>
        </div>
        {/* datas */}
        ...
        <div className="flex gap-4 flex-col mt-4">
          <small className="text-sm font-medium leading-none text-gray-500">
            {t("finetune.finetuneModelName")}
          </small>
          <Input
            maxLength={50}
            showCount
            className="max-w-[400px]"
            onChange={(e) => (resultRef.current["model_name"] = e.target.value)}
          ></Input>
        </div>
      </div>
      {/* datas */}
      <div className="border-b pb-4">
        <div className="flex gap-4 flex-col mt-4">
          ...
          <CreateTaskList
            onChange={(key, data) => (resultRef.current[key] = data)}
          ></CreateTaskList>
        </div>
      </div>
      {/* table */}
      <div className="border-b pb-4">
        <div className="flex gap-4 flex-col mt-4">
          <small className="text-sm font-medium leading-none text-gray-500">
            {t("finetune.trainingMethod")}
          </small>
          <div className="mt-1">
            <RadioGroup
              defaultValue="full"
              className="flex gap-6"
              onValueChange={(val) => (resultRef.current["method"] = val)}
            >
                  ...
                  <TableRow key={index}>
                    ...
                    <TableCell>
                      <Input
                        ...
                        onChange={(e) =>
                          (resultRef.current["extra_params"][item.name] =
                            e.target.value)
                        }
                      ></Input>
                    </TableCell>
