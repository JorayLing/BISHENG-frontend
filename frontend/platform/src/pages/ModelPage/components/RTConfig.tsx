...
export default function RTConfig({ open, onChange }) {
  ...
  const { services, showAdd, addItem, handleDel, create, setShowAdd } =
    useRTService(onChange);
            ...
            <TableBody>
              {services.map((el) => (
                <TableRow key={el.id}>
                  ...
                  <TableCell className="py-2 text-right">
                    <Button
                      variant="link"
                      className="h-8 rounded-full text-red-500 px-5"
              ...
              {showAdd && (
                <TableRow>
                  ...
                  <TableCell>
                    <Input ref={ftUrlRef} placeholder="IP:PORT"></Input>
                  </TableCell>
                  <TableCell>
                    <Button
                      variant="link"
                    ...
                    <Button
                      variant="link"
                      className="h-8 rounded-full text-gray-400"
...
const useRTService = (onChange) => {
  ...
  const addItem = (name, ftUrl) => {
    captureAndAlertRequestErrorHoc(
      addServiceApi(name, ftUrl).then((data) => {
        setServices([
          ...services,
          {
            id: data.id,
