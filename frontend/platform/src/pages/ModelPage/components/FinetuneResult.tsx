...
export default function FinetuneResult({
  id,
  training,
  isStop,
  failed,
  ...
  const loadData = (loop?) => {
    getTaskInfoApi(id).then((data) => {
      ...
      // 状态变更停止轮训
      loop &&
        finetune.status !== TaskStatus.TRAINING_IN_PROGRESS &&
        onChange(finetune.status);
    });
  };
      ...
      <div className="border-b pb-4">
        ...
        {/* cards */}
        {
          ...
          !failed && !isStop && report && (
            <div className="flex gap-4 mt-4">
              {processKeys.map((key) => (
                <Card className="flex-row w-[25%]" key={key}>
                  <CardHeader>
                    <CardTitle>{key}</CardTitle>
                    <CardDescription>
                      {training
                        ? "--"
                        : report[`predict_${key}`]?.toFixed(2) || "--"}
                      %
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="mt-4">
                    <div
                      className="radial-progress bg-gray-200 dark:bg-gray-950"
                      style={{
                        "--value": training ? 0 : report[`predict_${key}`],
                        "--size": "1.4rem",
                      }}
                      role="progressbar"
                    ></div>
