...
export default function CreateTaskList({ onChange }) {
  ...
  // 个人数据集
  const { userList, setUserList, handleChangeUserList, handleUploadUserList } =
    useUserList(onChange);
    ...
    <div>
      <div className="flex justify-between">
        <div>
          ...
          <Button variant="link" onClick={() => downloadJson(sampleData)}>
            {t("finetune.downloadSampleFile")}
          </Button>
        </div>
        <div className="flex gap-2 items-center">
          <Checkbox
            checked={isCustom}
            onCheckedChange={(val: boolean) => setIsCustom(val)}
          />
      ...
      <div className="border rounded-md p-2 overflow-y-auto max-h-[400px] mt-4 shadow-md bg-gray-100 dark:bg-gray-800">
        <p className="text-sm text-muted-foreground mt-4">
          {t("finetune.presetDatasets")}
        </p>
        {prsetList.length ? (
          prsetList.map((data, i) => (
            <div
              key={data.id}
              className="flex gap-4 items-center mt-2 h-8 hover:bg-gray-200 dark:hover:bg-gray-600 px-2"
            >
              <Checkbox
              ...
              <div className="flex ml-auto gap-4">
                ...
                {isCustom ? (
                  ...
                  <Badge variant="outline" className="text-sm">
                    {t("finetune.sampleSize")}:{data.sampleSize}
                  </Badge>
                )}
              </div>
            </div>
          ...
          <div className=" text-gray-400 text-sm mt-4 indent-2">
            {t("finetune.noData")}
          </div>
        )}
        <p className="text-sm text-muted-foreground mt-4">
          {t("finetune.userDatasets")}
        </p>
        {userList.length ? (
          userList.map((data) => (
            <div className="flex gap-4 items-center mt-2 h-8 hover:bg-gray-200 dark:hover:bg-gray-600 px-2">
              <Checkbox
                checked={data.checked}
                onCheckedChange={(val: boolean) =>
                  handleChangeUserList(data.id, "checked", val)
                }
              />
              <span className="text-sm">{data.name}</span>
              <div className="flex ml-auto gap-4">
...
export function usePresetList(onChange) {
  ...
  useEffect(() => {
    getPresetFileApi({ page_size: 400, page_num: 1, keyword: "" }).then((res) =>
      setPrsetList(res.data),
    );
  }, []);
