...
export default function UploadModal({
  ...
  const { end, tasks, onDrop, getResult } = useUpload(
    true,
    50,
    fileName,
    onUpload,
      ...
      <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="w-[460px]">
            ...
            <div className=" max-h-[300px] overflow-y-auto no-scrollbar mt-4">
              {tasks.map((task) => (
                <div key={task.id}>
                  <p
                    className={`max-w-[300px] overflow-hidden text-ellipsis whitespace-nowrap ${task.error && "text-red-400"}`}
                  >
                    {task.file.name}
                  </p>
                  <Progress
        ...
        <DialogFooter>
          ...
          <Button
            ...
            onClick={() => !loading && onSubmit(getResult())}
          >
            {loading && (
...
const useUpload = (open, maxSize, fileKey, uploadFunc) => {
  ...
  const onDrop = (acceptedFiles) => {
    ...
    errorFile.length &&
      setErrorData({
        title: t("prompt"),
        list: errorFile.map(
          (str) => `${t("code.file")}: ${str} ${t("code.sizeExceedsLimit")}`,
        ),
      });
  ...
  useEffect(() => {
    ...
    if (peddingTasks.length || awaitTasks.length) {
      ...
      awaitTasks
        .filter((e, i) => i < maxRequestCount - peddingTasks.length)
        .forEach((task) => {
          // 记录开始上传标记
          ...
          uploadFileWithProgress(task.file, (count) => {
            ...
            // 判断所有任务处理完成
            setEnd(
              filePathsRef.current.length + failNamesRef.current.length ===
