...
const PageChange = () => {
  ...
  useEffect(() => {
    getNextMarkChatApi({ action: "prev", chat_id: cid, task_id: id }).then(
      (res) => {
        setHasPrev(!!res);
        prevInfoRef.current = res;
      },
    );
    getNextMarkChatApi({ action: "next", chat_id: cid, task_id: id }).then(
      (res) => {
        setHasNext(!!res);
      ...
      <div className="bg-background-login px-4">
        <div className="flex justify-between items-center py-4">
          ...
          <RadioGroup
            ...
            disabled={!isSelf && status !== LabelStatus.Unlabeled}
          >
            <Label className="flex justify-center">
              <RadioGroupItem
                className="mr-2"
                disabled={!mark}
                value={LabelStatus.Unlabeled}
              />
              {t("label.unlabeled")}
            </Label>
            <Label className="flex justify-center">
