...
const useAppsOptions = () => {
  ...
  const loadApps = () => {
    ...
    getChatOnlineApi(page, keywordRef.current, -1).then((res: any) => {
      ...
      optionsRef.current =
        page === 1 ? newOptions : [...optionsRef.current, ...newOptions];
        ...
        <div className="flex flex-col gap-4 py-2">
          ...
          {count ? (
            <p className="text-sm text-gray-500">
              {t("label.unmarkedConversationCount")}: {count}
            </p>
          ) : null}
          <div className="">
            <label htmlFor="name" className="bisheng-label">
              {t("label.selectLabelers")}
            </label>
            <UsersSelect multiple value={users} onChange={setUsers} />
          </div>
        </div>
        <DialogFooter>
          <DialogClose>
    ...
    <div className="relative px-2 pt-4 h-full">
      <div className="h-full overflow-y-auto pb-20">
        <div className="flex justify-end gap-6">
          {["admin", "group_admin"].includes(user.role) && (
            ...
            {tasks.map((task) => (
              <TableRow key={task.id}>
                ...
                <TableCell className="break-all">
                  {(task.mark_process || []).map((name) => (
                    <p>{name}</p>
                  ))}
                </TableCell>
                <TableCell className="text-right">
                  <Link to={`/label/${task.id}`}>
                    <Button variant="link" className="px-0 pl-4">
                      {t("label.view")}
                    </Button>
                  </Link>
                </TableCell>
