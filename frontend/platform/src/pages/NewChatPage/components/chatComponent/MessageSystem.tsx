...
export default function MessageSystem({ data }) {
  ...
  const border = {
    ...
    processing: "border-cyan-600",
    answer: "border-lime-600",
    report: "border-slate-500",
    guide: "border-none",
  };
  ...
  return (
    <div className="py-1">
      <div
        className={`relative rounded-sm px-6 py-4 border text-sm dark:bg-gray-900 ${data.category === "guide" ? "bg-[#EDEFF6]" : "bg-slate-50"} ${border[data.category || "system"]}`}
      >
        {logMkdown}
        {data.category === "report" && (
          <Copy
            className=" absolute right-4 top-2 cursor-pointer"
            onClick={(e) => handleCopy(e.target.parentNode)}
          ></Copy>
        )}
      </div>
    </div>
