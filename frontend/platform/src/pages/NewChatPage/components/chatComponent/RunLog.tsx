...
export default function RunLog({ data }) {
  ...
  const [title, lost] = useMemo(() => {
    ...
    if (data.category === "flow") {
      const flow = assistantState.flow_list?.find(
        (flow) => flow.id === data.message.tool_key,
      );
      ...
      const tool = assistantState.tool_list?.find(
        (tool) => tool.tool_key === data.message.tool_key,
      );
      ...
      const knowledge = assistantState.knowledge_list?.find(
        (knowledge) => knowledge.id === parseInt(data.message.tool_key),
      );
      // if (!knowledge) throw new Error('调试日志无法匹配到使用的知识库详情，id:' + data.message.tool_key)

      title = knowledge
        ? `${data.end ? "已搜索" : "正在搜索"} ${knowledge.name}`
        : "知识库已被删除，无法获取知识库名";
    } else {
  ...
  // 没任何匹配的工具，隐藏
  if (
    assistantState.tool_list.length +
      assistantState.knowledge_list.length +
      assistantState.flow_list.length ===
    0
  )
    return null;
      ...
      <div className="rounded-sm border max-w-[90%]">
        ...
        >
          <div className="flex items-center font-bold gap-2 text-sm">
            {data.end ? (
              <ToastIcon type={lost ? "error" : "success"} />
            ) : (
        ...
        <div
          className={cname(
            "bg-[#F5F6F8] dark:bg-[#313336] px-4 py-2 overflow-hidden text-sm ",
            open ? "h-auto" : "h-0 p-0",
          )}
        >
          {data.thought.split("\n").map((line, index) => (
            <p className="text-md mb-1 text-muted-foreground" key={index}>
              {line}
            </p>
          ))}
        </div>
