...
export const useMessageStore = create<State & Actions>((set, get) => ({
  ...
  createWsMsg(data) {
    ...
    set((state) => {
      ...
      newChat.push({
        isSend: false,
        message: runLogsTypes.includes(data.category)
  ...
  updateCurrentMessage(wsdata, cover = false) {
    ...
    for (let i = messages.length - 1; i >= 0; i--) {
      ...
      if (isRunLog && messages[i].extra === wsdata.extra) {
        ...
        wsdata.type === "end_cover" &&
        messages[i].category === "tool"
      ) {
        currentMessageIndex = i;
        break;
      }
    ...
    if (isRunLog) {
      ...
      wsdata.message &&
      "reasoning_content" in wsdata.message
    ) {
    ...
    const newCurrentMessage = {
      ...
      reasoning_log,
      thought:
        currentMessage.thought + (wsdata.thought ? `${wsdata.thought}\n` : ""),
      files: wsdata.files || [],
    ...
    if (!isRunLog) {
      ...
      if (
        newCurrentMessage.end &&
        !(
          newCurrentMessage.files.length ||
          newCurrentMessage.thought ||
          newCurrentMessage.message
        )
      ) {
        messages.pop();
      }
