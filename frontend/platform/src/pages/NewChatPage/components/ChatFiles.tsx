...
export default function ChatFiles({ v, accepts, onChange }) {
  ...
  const handleFileChange = (e) => {
    ...
    if (invalidFiles.length > 0) {
      toast({
        variant: "info",
        description: invalidFiles.map(
          (file) =>
            `文件：${file.file.name}超过${appConfig.uploadFileMaxSize}M，已移除`,
        ),
      });
    }
      ...
      return uploadChatFile(v, file, (progress) => {
        ...
        .then((response) => {
          ...
          if (remainingUploadsRef.current === 0) {
            ...
            const uploadedFileIds = filesRef.current
              .filter((f) => f.id)
              .map((f) => ({ path: f.filePath, name: f.name }));
  ...
  return (
    <div className="relative z-10">
      ...
      {/* File Upload Button */}
      <div
        className="cursor-pointer mr-[12px] mt-1"
