 ...
 */
const ChatReportForm = forwardRef(
  ({ type = "chat", vid = 0, flow, onStart }, ref) => {
    ...
    useEffect(() => {
      // chat -》L1； diff -> 对比测试
      type === "chat"
        ? getVariablesApi({ flow_id: flow.flow_id || flow.id }).then((res) =>
            setItems(res),
          )
        : getVariablesApi({
            version_id: vid,
    ...
    const handleChange = (index, value) => {
      setItems((_items) =>
        _items.map((item, i) => (i === index ? { ...item, value } : item)),
      );
    };
    ...
    const handleStart = () => {
      ...
      const errors = items.reduce((res, el) => {
        ...
        if (
          el.type === VariableType.Text &&
          el.value.length > Number(el.maxLength)
        ) {
      ...
      const str = items
        .map(
          (el, i) =>
            `${el.name ? el.name + "：" : ""}${
              el.type === VariableType.File
                ? fileKindexVpath.current[i]
                : el.value
            }\n`,
        )
        .join("");
              ...
              <div className="mt-2">
                {item.type === VariableType.Text ? (
                  ...
                  <InputFileComponent
                    ...
                    fileTypes={["pdf"]}
                    suffixes={
                      flow.data.nodes.find((el) => el.id === item.nodeId)?.data
                        .node.template.file_path.suffixes || ["xxx"]
                    }
