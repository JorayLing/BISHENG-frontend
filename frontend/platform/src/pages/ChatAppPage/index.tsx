  ...
  const handlerSelectFlow = async (card) => {
    ...
    if (card) {
      ...
      return message({
        title: t("prompt"),
        variant: "warning",
        description: t("chat.pleaseSelectAnApp"),
      });
    }
  };
    ...
    <div className="flex h-full">
      <div className="h-full w-[220px] relative border-r">
        <div className="absolute flex top-0 w-full bs-chat-bg bg-background-main-content z-10 p-2">
          ...
          <div
            onClick={() => handlerSelectFlow(null)}
            id="newchat"
            className="flex justify-around items-center w-[200px] h-[48px] rounded-lg px-10 py-2 mx-auto text-center text-sm cursor-pointer bg-background-main-content hover:bg-gray-100 dark:hover:bg-gray-800 relative z-10"
          >
            <PlusBoxIcon className="dark:hidden"></PlusBoxIcon>
            <PlusBoxIconDark className="hidden dark:block"></PlusBoxIconDark>
            {t("chat.newChat")}
...
const useChatList = () => {
  ...
  useEffect(() => {
    if (messages.length > 0 && chatId === messages[0].chat_id) {
      ...
      // 有分割线取上一条
      if (latest.category === "divider")
        latest = messages[messages.length - 2] || {};
      setChatList((chats) =>
        chats.map((chat) =>
          chat.chat_id === chatId
            ? {
        ...
        chats.map((chat) =>
          chat.chat_id === flowChatId
            ? {
                ...
                latest_message: {
                  ...chat.latest_message,
                  message: (latest.message.msg || latest.message).substring(
                    0,
                    40,
                  ),
                },
    ...
    deleteChat: (id: string) => {
      ...
      captureAndAlertRequestErrorHoc(
        deleteChatApi(id).then((res) => {
          setChatList((oldList) =>
            oldList.filter((item) => item.chat_id !== id),
          );
        }),
      );
    },
