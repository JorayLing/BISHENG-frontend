...
export default function ChatPanne({
  customWsHost = "",
  appendHistory = false,
  data,
  version = "v1",
  ...
  const flowSate = useMemo(() => {
    ...
    // 是否群聊
    const isRoom = !!flow.data?.nodes.find(
      (node) => node.data.type === "AutoGenChain",
    );
    // 是否展示表单
    const isForm = !!flow.data?.nodes.find((node) =>
      ["VariableNode", "InputFileNode"].includes(node.data.type),
    );
    // 是否报表
    const isReport = !!flow.data?.nodes.find(
      (node) => "Report" === node.data.type,
    );
  ...
  return (
    <div className="flex-1 min-w-0 min-h-0 bs-chat-bg">
      ...
      {flow && (
        <div
          className={`w-full chat-box h-full relative px-6 ${type === AppNumType.SKILL ? "block" : "hidden"}`}
        >
          ...
          <ChatComponent
            ...
            inputForm={
              flowSate.isForm ? (
                <ChatReportForm flow={flow} onStart={sendReport} />
              ) : null
            }
          />
          {/* 强制提醒 */}
      ...
      {assistant && (
        <div
          className={`w-full chat-box h-full relative px-6 ${type === AppNumType.ASSISTANT ? "block" : "hidden"}`}
        >
          {/* {flow && <ChatPanne chatId={chatId} flow={flow} />} */}
          <div className="absolute flex top-2 gap-2 items-center z-10 bg-[rgba(255,255,255,0.8)] px-2 py-1 dark:bg-[#1B1B1B]">
            <TitleLogo url={assistant.logo} className="" id={assistant.id}>
              <AssistantIcon />
            </TitleLogo>
      ...
      {workflow && (
        <div
          className={`w-full chat-box h-full relative ${type === AppNumType.FLOW ? "block" : "hidden"}`}
        >
...
const useBuild = () => {
  ...
  async function handleBuild(flow: FlowType, chatId: string) {
    try {
      const errors = flow.data.nodes.flatMap((n: NodeType) =>
        validateNode(n, flow.data.edges),
      );
      if (errors.length > 0) {
        return toast({
