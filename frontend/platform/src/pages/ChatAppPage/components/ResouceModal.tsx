...
const Anwser = ({ id, msg, onInit, onAdd, fullScreen = false }) => {
  ...
  return (
    <div
      className="bg-gray-100 dark:bg-[#3C4048] rounded-md py-4 px-2 max-h-24 mb-4 overflow-y-auto"
      style={{ display: fullScreen ? "none" : "block" }}
    >
      <p
        ref={pRef}
        className="anwser-souce"
...
const ResultPanne = ({
  ...
  const loadFiles = () => {
    ...
    timer = setTimeout(() => {
      getSourceChunksApi(chatId, data.messageId, words.join(";")).then(
        (_files) => {
          setFiles(_files);
          // 默认打开第一个文件
  ...
  return (
    ...
    >
      {isMobile && (
        <div className="absolute top-0 left-4 z-50 bg-gray-100 dark:bg-gray-950 py-1 px-2 pb-2 rounded-md">
          {!collapse && (
            <span
              onClick={() => {
                setCollapse(true);
      ...
      {isMobile && (
        <div className="absolute top-0 right-4 z-10 bg-gray-100 dark:bg-gray-950 py-1 px-2 pb-2 rounded-md">
          <span onClick={closeDialog}>关闭</span>
        </div>
      )}
      {/* left */}
      {(!isMobile || !collapse) && (
        <div className="sm:w-[300px] bg-gray-100 dark:bg-[#3C4048] rounded-md py-4 px-2 h-full overflow-y-auto no-scrollbar w-[200px] max-h-[100%] sm:max-h-full absolute sm:static z-20 sm:z-auto">
          {/* label */}
            ...
            <div className="flex">
              ...
              <TooltipProvider delayDuration={100}>
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p className="w-[170px] break-words">
                      {t("chat.tooltipText")}
                    </p>
                  </TooltipContent>
                </Tooltip>
          ...
          <div className="flex flex-wrap gap-2 text-sm">
            {words.map((str, i) => (
              <div
                key={str}
                className="badge rounded-md px-2 badge-info h-[auto] gap-2 text-gray-600 bg-[rgba(53,126,249,.15)] dark:text-slate-50"
              >
                {str}
                <span
            ...
            {editCustomKey ? (
              <div className="badge badge-info cursor-pointer bg-[rgba(53,126,249,.15)]">
                <input
                  ref={inputRef}
                  id="taginput"
                  className="w-20 h-4 py-0 border-none outline-none bg-gray-50"
                  onKeyDown={(event) => {
              ...
              <div
                className="badge badge-info rounded-md px-2 cursor-pointer bg-[rgba(53,126,249,.86)] text-gray-50"
            ...
            {files.map((_file) =>
              _file.right ? (
                <div
                  ...
                  className={`group rounded-xl bg-[#fff] dark:bg-[#303134] hover-bg-gray-200 flex items-center px-4 mb-2 relative min-h-16 cursor-pointer ${file?.id === _file.id && "bg-gray-200"}`}
                >
                  <p className="text-sm break-all">{_file.fileName}</p>
                  <div className="absolute right-1 top-1 gap-2 hidden group-hover:flex">
                    {_file.parse_type === "uns" && _file.fileUrl && (
                      <Tip content={t("chat.downloadPDFTooltip")}>
                        <a
                          href="javascript:;"
                          onClick={(event) => {
                            downloadFile(
                              checkSassUrl(_file.fileUrl),
                              _file.fileName.replace(/\.[\w\d]+$/, ".pdf"),
                            );
                            event.stopPropagation();
                          }}
                        >
                          <Import
                            color="rgba(53,126,249,1)"
                            size={22}
                            strokeWidth={1.5}
                          ></Import>
                        </a>
                    ...
                    {_file.originUrl && (
                      <Tip content={t("chat.downloadOriginalTooltip")}>
                        <a
                          href="javascript:;"
                          onClick={(event) => {
                            downloadFile(
                              checkSassUrl(_file.originUrl),
                              _file.fileName,
                            );
                            event.stopPropagation();
                          }}
                  ...
                  <span className="absolute right-1 bottom-1 text-blue-400 text-sm">
                    {_file.score}
                  </span>
                </div>
              ) : (
                <div
                  key={_file.id}
                  className={`msk group rounded-xl bg-[#fff] hover-bg-gray-200 flex items-center px-4 mb-2 relative min-h-16 cursor-pointer ${file?.id === _file.id && "bg-gray-200"}`}
                >
...
export const ResouceContent = ({ data, setOpen, fullScreen = false }) => {
  ...
  const handleAddWord = (word: string) => {
    // 去重 更新
    setKeywords((oldWords) =>
      oldWords.find((wd) => wd === word) ? oldWords : [...oldWords, word],
    );
  };
