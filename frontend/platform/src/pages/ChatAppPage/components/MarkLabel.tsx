...
function DragItem({ className = "", data, children, onCancel }) {
  return (
    <div
      className={cname(
        "h-7 w-32 relative rounded-xl border flex place-items-center",
        className,
      )}
    >
      <CircleX
      ...
      <div className="ml-2 truncate">{children}</div>
    </div>
  );
}
...
export default function MarkLabel({ open, home, onClose }) {
  ...
  const handleConfirm = async () => {
    await captureAndAlertRequestErrorHoc(
      updateHomeLabelApi(selected.map((s) => s.value)),
    );
    onClose(true);
  };
  ...
  const handleSelect = (id) => {
    setLabels((pre) => {
      const newData = pre.map((l) =>
        l.value === id ? { ...l, selected: !l.selected } : l,
      );
  ...
  const handleDelete = (id) => {
    ...
    setLabels((pre) =>
      pre.map((d) => (d.value === id ? { ...d, selected: !d.selected } : d)),
    );
  };
          ...
          <div className="border-l text-gray-500">
            <div className="ml-4">
              <span className="text-md font-bold">
                {t("chat.selected")}：{selected.length}/10
              </span>
              <DragDropContext
                onDragEnd={handleDragEnd}
                onDragStart={() => setFlag(true)}
                onDragUpdate={() => setFlag(true)}
              >
