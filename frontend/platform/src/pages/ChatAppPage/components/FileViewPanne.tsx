...
export default function FileViewPanne({ file }) {
  ...
  const labels = useMemo(() => {
    ...
    setPositon([
      file.chunks[0].box[0].page,
      file.chunks[0].box[0].bbox[1] || 0,
    ]);
    return map;
  }, [file.chunks]);
  ...
  return (
    <div
      className="flex-1 bg-gray-100 rounded-md py-4 px-2 relative"
      onContextMenu={(e) => e.preventDefault()}
    >
      <div className="h-full overflow-auto">{fileView()}</div>
      {/* chunk menu */}
      {showP && (
        <div className="absolute left-[0px] rounded-sm p-4 px-0 top-[50%] translate-y-[-50%] max-2xl:scale-75 origin-top-left">
          <p className="mb-1 text-sm font-bold text-center rounded-sm bg-[rgb(186,210,249)] text-blue-600">
            {t("chat.sourceTooltip")}
          </p>
