...
export default function HomePage({ onSelect }) {
  ...
  const renderChatTypeBadge = (type) => (
    <Badge
      className={`absolute right-0 bottom-0 rounded-none rounded-br-md ${
        type === 1 ? "bg-gray-950" : type === 5 ? "bg-[#fdb136]" : ""
      }`}
    >
  ...
  return (
    <div
      className="h-full overflow-hidden bs-chat-bg"
...
const HeaderSection = ({ BASE_IMAGE_URL, t }) => (
  <div className="flex justify-center place-items-center gap-20">
    ...
    <p className="text-2xl leading-[50px] dark:text-[#D4D4D4]">
      ...
      <br />
      {t("chat.start")}
      <b className="dark:text-[#D4D4D4] font-semibold">
        {t("chat.wenqingruijian")}
      </b>
    </p>
  </div>
);
