...
const ChatItem = ({
  ...
  return (
    <div
      key={chat.chat_id}
      className={`group item w-full rounded-lg mt-2 p-4 relative hover:bg-[#EDEFF6] cursor-pointer dark:hover:bg-[#34353A] ${
        location
    ...
    >
      <div className="flex place-items-center space-x-3">
        <div className="inline-block bg-purple-500 rounded-md">
          <TitleLogo url={chat.logo} id={chat.flow_id}>
...
export default function ChatPro({ type = AppNumType.SKILL }) {
  ...
  // 获取消息状态用于监听
  const { messages } =
    type === AppNumType.ASSISTANT ? useMessageStore() : useFlowMessageStore();
      ...
      const initializeFlow = async () => {
        try {
          ...
          const defaultWorkflow = {
            ...
            logo: "",
            description:
              type === AppNumType.ASSISTANT ? "助手描述" : "工作流描述",
  ...
  useEffect(() => {
    if (messages && messages.length > 0 && selectChat?.chatId) {
      ...
      if (
        latestMessage.is_bot &&
        (latestMessage.type === "end" ||
          latestMessage.type === "over" ||
          latestMessage.end) &&
  ...
  const handlerSelectFlow = async (workflowInfo = null) => {
    ...
    if (type === AppNumType.ASSISTANT) {
      ...
      description =
        ...
        existingChat?.flow_description ||
        "";
    } else {
      // Flow API返回的是description字段
      name = _workflow?.name || existingChat?.flow_name || `工作流 ${flowId}`;
      description =
        _workflow?.description || existingChat?.flow_description || "";
    }
    ...
    <div className="flex h-full">
      <div className="h-full w-[220px] relative border-r">
        <div className="absolute flex top-0 w-full bs-chat-bg bg-background-main-content z-10 p-2">
          ...
          >
            <PlusBoxIcon
              className={`dark:hidden ${isCreatingChat ? "opacity-50" : ""}`}
            />
            <PlusBoxIconDark
              className={`hidden dark:block ${isCreatingChat ? "opacity-50" : ""}`}
            />
            {isCreatingChat
              ? t("chat.creating", "创建中...")
              : t("chat.newChat")}
        ...
        <div
          ref={chatsRef}
          className="scroll h-full overflow-y-scroll no-scrollbar p-2 pt-14"
        >
          {chatList.length === 0 && (
            <div className="text-center text-gray-500 mt-4">暂无聊天记录</div>
          )}
          {chatList
  ...
  const refreshSpecificChat = async (targetChatId: string) => {
    try {
      ...
      if (updatedChat) {
        console.log(
          "找到更新的聊天信息:",
          updatedChat.latest_message?.message ||
            updatedChat.earliest_message?.remark,
        );
    ...
    addChat: (chat) => {
      ...
      setChatList((prev) => {
        ...
        const filteredList = prev.filter(
          (existingChat) => existingChat.chat_id !== chat.chat_id,
        );
