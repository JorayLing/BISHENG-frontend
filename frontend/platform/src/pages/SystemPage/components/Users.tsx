...
function UsersFilter({ options, onChecked, nameKey, placeholder, onFilter }) {
  ...
  const _options = useMemo(() => {
    ...
    return options.filter((a) =>
      a[nameKey].toUpperCase().includes(searchKey.toUpperCase()),
    );
  }, [searchKey, options]);
  // 重置
  const reset = () => {
    ...
    >
      <PopoverTrigger>
        ...
        <FilterIcon
          ...
          className={_value.length ? "text-primary ml-3" : "text-gray-400 ml-3"}
        />
      </PopoverTrigger>
      <PopoverContent>
        <FilterUserGroup
    ...
    bsConfirm({
      ...
      onOk(next) {
        captureAndAlertRequestErrorHoc(
          disableUserApi(user.user_id, 1).then((res) => {
            reload();
          }),
        );
        next();
  ...
  const operations = (el) => {
      ...
      return (
        <div>
          ...
          <Button
            variant="link"
            className="px-0 pl-4"
            onClick={() => userPwdModalRef.current.open(el.user_id)}
          >
            {t("system.resetPwd")}
          </Button>
          <Button variant="link" disabled className="text-red-500 px-0 pl-4">
            {t("disable")}
          </Button>
        </div>
      );
    ...
    return (
      <div>
        ...
        <Button
          variant="link"
          disabled={user.user_id === el.user_id}
          onClick={() => setCurrentUser(el)}
          className="px-0"
        >
          {t("edit")}
        </Button>
        ...
        {el.delete === 1 ? (
          <Button
            variant="link"
            onClick={() => handleEnableUser(el)}
            className="text-green-500 px-0 pl-4"
          >
            {t("enable")}
          </Button>
        ) : (
          <Button
            ...
            className="text-red-500 px-0 pl-4"
          >
            {t("disable")}
          </Button>
        )}
        ...
        <Table className="mb-[50px]">
          ...
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right w-[164px]">
                {t("operations")}
              </TableHead>
            </TableRow>
          </TableHeader>
            ...
            {users.map((el: any) => (
              <TableRow key={el.id}>
                ...
                <TableCell className="break-all">
                  {(el.groups || []).map((el) => el.name).join(",")}
                </TableCell>
                <TableCell className="break-all">
                  {(el.roles || []).map((el) => el.name).join(",")}
                </TableCell>
