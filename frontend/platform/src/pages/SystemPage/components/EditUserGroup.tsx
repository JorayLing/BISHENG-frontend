...
function FlowRadio({ limit, onChange }) {
  ...
  return (
    <div>
      <RadioGroup
        className="flex space-x-2 h-[20px] items-center"
        value={status}
      ...
      >
        <div>
          <Label className="flex justify-center">
            <RadioGroupItem className="mr-2" value={LimitType.UNLIMITED} />
            {t("system.unlimited")}
          </Label>
        </div>
        <div>
          <Label className="flex justify-center">
            <RadioGroupItem className="mr-2" value={LimitType.LIMITED} />
            {t("system.limit")}
          </Label>
        </div>
...
function FlowControl({ groupId, type, onChange }) {
  ...
  const map = {
    3: {
      name: t("build.assistantName"),
      label: t("system.AssistantFlowCtrl"),
      placeholder: t("system.assistantName"),
    },
    2: {
      name: t("skills.skillName"),
      label: t("system.SkillFlowCtrl"),
      placeholder: t("skills.skillName"),
    },
    5: {
      name: t("build.workFlowName"),
      label: t("system.flowCtrl"),
      placeholder: t("build.workFlowName"),
    },
  };
  const { name, label, placeholder } = map[type];
  const { page, pageSize, data, total, setPage, search, refreshData } =
    useTable({ pageSize: 10 }, (params) =>
      getGroupFlowsApi(
        params.page,
        params.pageSize,
        type,
            ...
            {data.map((i: any) => (
              <TableRow key={i.id}>
                ...
                <TableCell className="pt-4">
                  <FlowRadio
                    limit={i.limit}
                    onChange={(val) => handleChange(val, i.id)}
                  ></FlowRadio>
                </TableCell>
              </TableRow>
            ))}
...
export default function EditUserGroup({ data, onBeforeChange, onChange }) {
  ...
  const handleSave = async () => {
    ...
    if (!form.groupName) {
      ...
      return toast({
        title: t("prompt"),
        description: t("system.groupNameRequired"),
        variant: "error",
      });
    }
    if (form.groupName.length > 30) {
      ...
      return toast({
        title: t("prompt"),
        description: t("system.groupNamePrompt"),
        variant: "error",
      });
    }
    ...
    if (flag) {
      ...
      return toast({
        title: t("prompt"),
        description: t("system.groupNameExists"),
        variant: "error",
      });
    }

    // 过滤系统管理员
    const users = selected.filter(
      (item) => !lockOptions.some((id) => id === item.value),
    );

    const res: any = await (data.id
      ? updateUserGroup(data.id, form, users) // 修改
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const users =
        data.group_admins?.map((d) => ({
          label: d.user_name,
          value: d.user_id,
        })) || [];
      const defaultUsers = res.map((d) => ({
        label: d.user_name,
        value: d.user_id,
      }));
  ...
  return (
    <div className="max-w-[630px] mx-auto pt-4 h-[calc(100vh-128px)] overflow-y-auto pb-10 scrollbar-hide">
      <div className="font-bold mt-4">
        <p className="text-xl mb-4">{t("system.groupName")}</p>
        <Input
          placeholder={t("system.userGroupName")}
          required
      ...
      {appConfig.isPro && (
        <>
          <div className="font-bold mt-12">
            ...
            <FlowRadio
              limit={form.groupLimit}
              onChange={(f) => setForm({ ...form, groupLimit: f })}
            ></FlowRadio>
          </div>
          <div className="mt-12">
            <FlowControl
      ...
      <div className="flex justify-center items-center absolute bottom-0 w-[630px] h-[8vh] gap-4 mt-[100px] bg-background-login">
        <Button variant="outline" className="px-16" onClick={onChange}>
          {t("cancel")}
        </Button>
        <Button className="px-16" onClick={handleSave}>
          {t("save")}
        </Button>
      </div>
    </div>
