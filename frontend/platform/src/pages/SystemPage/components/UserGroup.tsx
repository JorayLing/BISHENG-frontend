...
export default function UserGroups() {
  ...
  const loadData = async () => {
    const res: any = await (
      appConfig.isPro ? getUserGroupsProApi : getUserGroupsApi
    )();
    ...
    bsConfirm({
      ...
      onOk(next) {
        captureAndAlertRequestErrorHoc(
          delUserGroupApi(userGroup.id).then(loadData),
        );
        next();
      },
    });
  ...
  const checkSameName = (name: string) => {
    return userGroups.find(
      (ug) => ug.group_name === name && ug.id !== userGroup.id,
    );
  };
            ...
            {userGroups.map((ug: any) => (
              <TableRow key={ug.id}>
                ...
                <TableCell className="break-all">
                  {(ug.admin_user || ug.group_admins)
                    .map((el) => el.user_name)
                    .join(",")}
                </TableCell>
                {appConfig.isPro && (
                  <TableCell>
                    {ug.group_limit ? t("system.limit") : t("system.unlimited")}
                  </TableCell>
                )}
                ...
                <TableCell className="text-right">
                  <Button
                    ...
                    onClick={() =>
                      setUserGroup({
                        ...ug,
                        group_admins: ug.group_admins.slice(
                          defaultAdminsRef.current.length,
                        ),
                      })
                    }
                    className="px-0 pl-6"
                  ...
                  <Button
                    variant="link"
                    disabled={ug.id === 2}
                    onClick={() => handleDelete(ug)}
                    className="text-red-500 px-0 pl-6"
                  >
                    {t("delete")}
                  </Button>
                </TableCell>
