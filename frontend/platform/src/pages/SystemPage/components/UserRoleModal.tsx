...
export default function UserRoleModal({ user, onClose, onChange }) {
    ...
    if (user) {
      ...
      const items = groups.map((item) => {
        return {
          ...
          groupId: item.id,
          roles: roles
            .filter((role) => role.group_id === item.id)
  ...
  const handleSave = async () => {
    ...
    if (items.some((item) => item.roles.length === 0))
      return message({
        title: t("prompt"),
        variant: "warning",
        description: t("system.selectRole"),
      });
    if (items.length === 0)
      return message({
        title: t("prompt"),
        variant: "warning",
        description: t("system.selectGroup"),
      });
    captureAndAlertRequestErrorHoc(
      updateUserRoles(
        user.user_id,
        items.reduce((res, item) => [...res, ...item.roles], []),
      ),
    );
    captureAndAlertRequestErrorHoc(
      updateUserGroups(
    ...
    >
      <DialogContent className="sm:max-w-[625px]">
        ...
        <div className="max-h-[520px] py-1 overflow-y-auto flex flex-col gap-2">
          {roleItems.map((item, i) => (
            <UserRoleItem
              ...
              onDelete={() =>
                setRoleItems(roleItems.filter((el, index) => index !== i))
              }
            />
          ))}
        </div>
        ...
        <DialogFooter>
          ...
          <Button className="px-16 h-10 w-[120px]" onClick={handleSave}>
            {t("save")}
          </Button>
        </DialogFooter>
      </DialogContent>
