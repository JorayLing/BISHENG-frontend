...
export default function Roles() {
  ...
  const checkSameName = useCallback(
    (name: string) => {
      return state.roles.find(
        (_role) => _role.role_name === name && state.role?.id !== _role.id,
      );
    },
  ...
  const options = useMemo(() => {
    ...
    return state.groups.filter(
      (group) =>
        group.label.toUpperCase().includes(keyWord.toUpperCase()) ||
        ...
        <div className="flex justify-between">
          <div className="flex items-center">
            ...
            <SelectSearch
              value={state.group}
              options={options}
              selectPlaceholder={t("system.defaultGroup")}
              inputPlaceholder={t("log.selectUserGroup")}
              selectClass="w-[180px] inline-flex ml-2"
              contentClass="max-w-[180px] break-all"
              onOpenChange={(open) => {
                !open && setKeyWord("");
              }}
              onValueChange={(value) => {
          ...
          <div className="flex gap-6 items-center justify-between">
            ...
            <Button
              ...
              onClick={() => dispatch({ type: "SET_ROLE", payload: {} })}
            >
            ...
            {state.roles.map((el) => (
              <TableRow key={el.id}>
                ...
                <TableCell className="text-right">
                  <Button
                    variant="link"
                    onClick={() => dispatch({ type: "SET_ROLE", payload: el })}
                    className="px-0 pl-6"
                  >
                    {t("edit")}
                  </Button>
