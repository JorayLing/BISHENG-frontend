...
export default function CreateUser({ open, onClose, onSave }) {
  ...
  const handleConfirm = async () => {
    ...
    if (!PWD_RULE.test(form.password))
      errors.push(t("system.passwordRequirements"));
    if (items.every((item) => item.roles.length === 0))
      errors.push(t("system.roleRequired"));
    if (errors.length > 0)
      return message({
        title: t("prompt"),
        description: errors,
    ...
    captureAndAlertRequestErrorHoc(
      createUserApi(form.user_name, encryptPwd, group_roles).then(() => {
        copyText(
          `${t("system.username")}: ${form.user_name}，${t("system.initialPassword")}: ${form.password}`,
        ).then(() =>
          message({
            title: t("prompt"),
            description: t("system.userCreationSuccess"),
            variant: "success",
          }),
        );
        onClose(false);
  ...
  return (
    <Dialog open={open} onOpenChange={(b) => onClose(b)}>
      <DialogContent className="sm:max-w-[625px]">
        ...
        <div className="flex flex-col gap-4 mb-4">
          <div>
            <Label htmlFor="user" className="bisheng-label">
              {t("system.username")}
            </Label>
            <Input
              id="user"
          ...
          <div>
            <Label htmlFor="password" className="bisheng-label">
              {t("system.initialPassword")}
            </Label>
            <PasswordInput
              id="password"
              value={form.password}
          ...
          <div className="flex flex-col gap-2">
            <Label className="bisheng-label">
              {t("system.userGroupRoleSelection")}
            </Label>
            <div className="max-h-[520px] overflow-y-auto flex flex-col gap-2">
              {items.map((item, index) => (
                <UserRoleItem
                  ...
                  selectedRoles={[]}
                  onChange={(groupId, roles) =>
                    handleChangeRoleItems(index, groupId, roles)
                  }
                  onDelete={() =>
            ...
            <Button
              ...
              onClick={() =>
                setItems((items) => [
                  ...items,
                  { key: generateUUID(8), groupId: "", roles: [] },
                ])
              }
            >
              <Plus />
        ...
        <DialogFooter>
          ...
          <Button className="px-16 h-10 w-[120px]" onClick={handleConfirm}>
            {t("confirm")}
          </Button>
        </DialogFooter>
      </DialogContent>
