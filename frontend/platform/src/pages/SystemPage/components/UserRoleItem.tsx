...
export default function UserRoleItem({
  ...
  const handleSearch = (e) => {
    ...
    const newGroups = groupsRef.current.filter(
      (g) =>
        g.label.toUpperCase().includes(keyword.toUpperCase()) ||
  ...
  return (
    <div className="grid grid-cols-[44%,44%,5%] gap-4">
      <SelectSearch
        contentClass="max-w-[260px] break-all"
        selectPlaceholder={t("system.userGroupsSel")}
        selectClass="h-[50px]"
      ...
      ></MultiSelect>
      {showDel && (
        <Button variant="ghost" size="icon" className="mt-2" onClick={onDelete}>
          <DelIcon />
        </Button>
      )}
    </div>
