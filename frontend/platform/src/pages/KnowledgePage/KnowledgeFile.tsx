...
function CreateModal({ datalist, open, setOpen, onLoadEnd }) {
  ...
  // Fetch model data
  useEffect(() => {
    Promise.all([getKnowledgeModelConfig(), getModelListApi()])
      .then(([config, data]) => {
  ...
  const handleCreate = async (e, isImport = false) => {
    ...
    if (datalist.find((data) => data.name === name))
      errorlist.push(t("lib.nameExists"));
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-[625px]">
        ...
        <DialogFooter>
          <DialogClose>
            <Button variant="outline" className="px-8 h-8">
              {t("cancel")}
            </Button>
          </DialogClose>
          <Button
    ...
    <div className="relative">
      ...
      <div className="h-[calc(100vh-128px)] overflow-y-auto pb-20">
        <div className="flex justify-end gap-4 items-center absolute right-0 top-[-44px]">
          <SearchInput
            placeholder="知识库或文件名称"
            onChange={(e) => search(e.target.value)}
          />
          <Button className="px-8 text-[#FFFFFF]" onClick={() => setOpen(true)}>
            {t("create")}
          </Button>
        </div>
        <Table>
                ...
                >
                  ...
                  {(el.copiable || user.role === "admin") &&
                    (el.state === 1 ? (
                      ...
                      <Button variant="link" className="px-0 pl-2" disabled>
                        {t("lib.copying")}
                      </Button>
                    ))}
                  {el.copiable ? (
                    <Button
                      ...
                      className="text-red-500 px-0 pl-2"
                    >
                      {t("delete")}
                    </Button>
                  ) : (
                    <Button variant="link" className=" text-gray-400 px-0 pl-2">
                      {t("delete")}
                    </Button>
                  )}
                </TableCell>
