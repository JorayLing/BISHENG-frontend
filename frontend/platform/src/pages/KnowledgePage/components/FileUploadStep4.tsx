...
export default function FileUploadStep4({ data }) {
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(
      pollFilesStatus,
      3000 + fileIdsRef.current.length * 10,
    );
  ...
  const handleCreateFlow = async (params) => {
    ...
    const flow = await getKnowledgeDefaultFlowTemplate(
      kid,
      details[0].name,
      model.model_id,
    );
  ...
  return (
    <div className={`max-w-[1400px] mx-auto px-20 pt-4 relative`}>
      <div className="flex gap-4">
        <div className="flex-1">
          <h1 className="text-3xl text-primary mt-2">
            {finish ? "文档数据解析已完成" : "文档数据正在准备中"}
          </h1>
          ...
          <div className="overflow-y-auto mt-4 space-y-2 pb-10 max-h-[calc(100vh-400px)]">
            {files.map((item) => (
              <ProgressItem analysis key={item.id} item={item} />
            ))}
          </div>
          ...
          <div className="w-96 pt-24">
            <CardComponent
              ...
              description={
                <p>
                  <p>
                    文档解析完成后。使用预制的知识库问答模版建立智能体，并测试问答效果
                  </p>
                </p>
              }
