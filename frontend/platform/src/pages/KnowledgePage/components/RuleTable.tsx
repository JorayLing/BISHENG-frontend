    ...
    <div className="flex-1 flex flex-col relative max-w-[760px] mx-auto">
      ...
      >
        <div className="flex justify-end items-center gap-2">
          <Checkbox
            checked={applyEachCell}
            onCheckedChange={setApplyEachCell}
          />
        ...
        {applyEachCell ? (
                ...
                <Accordion type="single" collapsible className="w-full mb-4">
                  {tableFils.map((file) => (
                    <AccordionItem
                      key={file.id}
                      value={file.id}
                      className="border border-gray/80 rounded-xl mb-2 hover:border-primary hover:shadow-lg"
                    >
                      {/* 下拉触发按钮 */}
                      <AccordionTrigger
                        hoverable
                        className="p-0 cursor-pointer relative overflow-hidden flex flex-row-reverse justify-between"
                      >
                        <p className="flex gap-2 p-2 items-center relative">
                          <FileIcon
                            type="xls"
                            className="size-[30px] min-w-8"
                          />
          ...
          <div className="space-y-4 p-4 border rounded-lg">
            ...
            <div className="flex flex-col gap-4">
              <ItemForm
                data={cellGeneralConfig}
                setData={(key, value) =>
                  setCellGeneralConfig((prev) => ({
                    ...prev,
                    [key]: value,
                  }))
                }
