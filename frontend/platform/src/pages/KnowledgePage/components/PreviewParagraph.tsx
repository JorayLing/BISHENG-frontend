...
export const MarkdownView = ({ noHead = false, data }) => {
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md hover:border-primary transition-shadow w-full">
      {!noHead && (
        <p className="text-sm text-gray-500 flex gap-2 mb-1">
...
const VditorEditor = forwardRef(
  ({ defalutValue, hidden, onBlur, onChange }, ref) => {
    ...
    return (
      <div
        ref={domRef}
        className={`${hidden ? "hidden" : ""} overflow-y-auto border-none file-vditor`}
      ></div>
    );
  },
);
...
const EditMarkdown = ({
  ...
  return (
    <div
      className={cn(
        "group p-4 py-3 bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md hover:border-primary transition-shadow w-full",
      ...
      <div className="text-sm text-gray-500 flex gap-2 justify-between mb-1">
        <div className="flex gap-2 items-center">
          ...
          <div className="flex gap-2 justify-center items-center">
            {fileSuffix === "pdf" && appConfig.enableEtl4lm && (
              <Tip content={"点击定位到原文件"} side={"top"}>
                <Button
                  size="icon"
                  variant="ghost"
                  className={cn(
                    "size-6 text-primary opacity-0 group-hover:opacity-100",
                  )}
            ...
            {edit ? (
              <div
                className={cn(
                  "size-6 text-primary flex justify-center items-center rounded-sm cursor-pointer opacity-0 group-hover:opacity-100",
                  edit && "bg-primary text-gray-50",
                )}
...
// 分段结果列表
export default function PreviewParagraph({
  fileId,
  previewCount,
  edit,
  fileSuffix,
  ...
  const [
    ...
    setSelectedChunkDistanceFactor,
  ] = useKnowledgeStore((state) => [
    state.selectedChunkIndex,
    state.setSelectedChunkIndex,
    state.setSelectedChunkDistanceFactor,
  ]);
  useEffect(() => {
  ...
  return (
    <div className="w-full pt-3 pb-10 relative ">
      ...
      <div
        ref={containerRef}
        className="h-[calc(100vh-284px)] overflow-y-auto"
      ...
      >
        <div className="space-y-6">
          ...
          {!(chunks.length || loading) && (
            <div className="p-4 bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md hover:border-primary transition-shadow text-sm text-gray-500 flex gap-2 mb-1">
              文档解析失败
            </div>
          )}
        </div>
