    ...
    <div>
      <Table>
        ...
        <TableBody>
          {dataList.map((el) => {
            ...
            return (
              <TableRow key={generateUUID(4)}>
                <TableCell className="font-medium">{mainQuestion}</TableCell>
          ...
          <SimilarityProblemModal
            ref={similarityQuestions}
            questions={questions}
          />
        </TableBody>
      </Table>
...
// 导入Qa
export const ImportQa = forwardRef(function (
  { knowlageId, onChange }: any,
  ref,
) {
  const { t } = useTranslation("knowledge");
  const [open, setOpen] = useState(false);
  ...
  const handleSubmit = async () => {
    ...
    if (errorLines.length) {
      message({
        variant: "warning",
        description: t("errorMsg", { value: errorLines.length }),
      });
    } else {
      ...
      <DialogContent className="sm:max-w-[825px]">
        ...
        <div>
          ...
          <div className="flex flex-col gap-4 py-2">
            <SimpleUpload
              filekey="file"
              uploadUrl={"/hjapi/v1/knowledge/upload"}
              accept={["xls", "xlsx"]}
