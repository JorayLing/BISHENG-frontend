...
const initialStrategies = [
  ...
  },
  {
    ...
    regex: "\\n",
    position: "after",
    rule: "单换行后切分，用于分隔普通换行",
  },
];

export default function FileUploadStep2({
  step,
  resultFiles,
  isSubmitting,
  onNext,
  ...
  const setSelectedChunkIndex = useKnowledgeStore(
    (state) => state.setSelectedChunkIndex,
  );

  const displayMode: DisplayModeType = useMemo(() => {
    const hasTableFiles = resultFiles.some((file) => file.fileType === "table");
    const hasDocumentFiles = resultFiles.some(
      (file) => file.fileType === "file",
    );
    return hasTableFiles && !hasDocumentFiles
      ? DisplayModeType.OnlyTables
      : !hasTableFiles && hasDocumentFiles
        ? DisplayModeType.OnlyDocuments
  ...
  const vildateCell = () => {
    if (
      applyEachCell
        ? rules.fileList.some(
            (file) =>
              file.excelRule.append_header &&
              Number(file.excelRule.header_start_row) >
                Number(file.excelRule.header_end_row),
          )
        : cellGeneralConfig.append_header &&
          Number(cellGeneralConfig.header_start_row) >
            Number(cellGeneralConfig.header_end_row)
    ) {
      return toast({
        variant: "warning",
      ...
      <div className="flex flex-row justify-center gap-4">
        ...
        {step === 2 && (
          ...
          >
            <Tabs
              defaultValue={
                displayMode === DisplayModeType.Mixed ? "file" : displayMode
              }
                ...
                {displayMode === DisplayModeType.Mixed ? (
                  <TabsList className="">
                    <TabsTrigger
                      ...
                      value="file"
                      className="roundedrounded-xl"
                    >
                      {t("defaultStrategy")}
                    </TabsTrigger>
                    <TabsTrigger id="knowledge_table_tab" value="table">
                      {t("customStrategy")}
                    </TabsTrigger>
                  </TabsList>
                ) : (
...
const useFileProcessingRules = (initialStrategies, resultFiles, kid) => {
  ...
  useEffect(() => {
    const [separator, separatorRule] = strategies.reduce(
      ([_separator, _separatorRule], strategy) => {
        const { regex, position } = strategy;
        return [
          [..._separator, regex],
          [..._separatorRule, position],
        ];
      },
      [[], []],
    );
