...
const ParagraphEdit = ({
  ...
  const handleSave = async () => {
    ...
    const promise = chunks
      ? updatePreviewChunkApi({
          knowledge_id: Number(id),
          file_path: oriFilePath,
          chunk_index: chunkId,
          text: _value,
  ...
  const handleSelectLabels = (lbs) => {
    ...
    selectLabels.forEach((item) => {
      arr = arr.map((el) =>
        el.id === item.id ? { ...el, active: item.active } : el,
      );
    });
    setData(arr);
  ...
  const handleOvergap = () => {
    ...
    data.forEach((item, index) => {
      if (typeof labelTextRef.current[item.id] === "string")
        return window.alter("文件已失效，传个新的在测试");
      if (item.active) {
  ...
  const fileView = () => {
    ...
    switch (suffix) {
      ...
      case "txt":
        return <TxtFileViewer filePath={previewFileUrl} />;
      case "md":
        return <TxtFileViewer markdown filePath={previewFileUrl} />;
      case "html":
        return <TxtFileViewer html filePath={previewFileUrl} />;
      case "doc":
      case "docx":
        return <DocxPreview filePath={previewFileUrl} />;
      case "png":
      case "jpg":
      case "jpeg":
      case "bmp":
  ...
  return (
    <div className="flex px-4 py-2 select-none">
      ...
      <div className="relative" style={{ width: leftPanelWidth }}>
        ...
        />
        {!value && (
          <p className="absolute left-0 text-red-500 text-xs mt-2">
            {t("inputNotEmpty")}
          </p>
        )}
        {!isUns && (
          <div className="flex justify-end gap-4">
            <Button className="px-6 h-8" variant="outline" onClick={onClose}>
              {t("cancel", { ns: "bs" })}
            </Button>
            <Button
              className="px-6 h-8"
        ...
        <>
          ...
          <div className="flex-1 min-w-0 w-0">
            ...
            <div className="flex justify-between items-center relative h-10 mb-2 text-sm">
              <span>{fileName}</span>
              <div
                className={`${labelChange ? "" : "hidden"} flex items-center`}
              >
                <Info className="mr-1 text-red-500" />
                <span className="text-red-500">
                  {t("segmentRangeDetected")}
                </span>
                <span
                  className="text-primary cursor-pointer"
                  onClick={handleOvergap}
                >
                  {t("overwriteSegment")}
                </span>
              </div>
            ...
            <div className="bg-gray-100 relative">
              {showPos && value && Object.keys(labels).length !== 0 && (
                <Button
                  className="absolute top-2 right-2 z-10 bg-background"
                  variant="outline"
                  onClick={() => setRandom(Math.random() / 10000)}
                >
...
const useDragSize = (full) => {
  const [leftPanelWidth, setLeftPanelWidth] = useState(
    full ? "100%" : window.innerWidth * 0.4,
  );
