...
export default function Files({ onPreview }) {
  ...
  const handleRetry = (objs) => {
    captureAndAlertRequestErrorHoc(
      retryKnowledgeFileApi({ file_objs: objs }).then((res) => {
        reload();
      }),
    );
  };
  ...
  const dataSouce = useMemo(() => {
    return datalist.map((el) => {
      ...
      return {
        ...el,
        strategy: [
          data.length > 2 ? data.slice(0, 2).join(",") : "",
          data.join(","),
        ],
      };
    });
  ...
  const splitRuleDesc = (el) => {
    ...
    return ["XLSX", "XLS", "CSV"].includes(suffix)
      ? `每 ${excel_rule.slice_length} 行作为一个分段`
      : el.strategy[1].replace(/\n/g, "\\n");
  };
              ...
              <TableHead className="flex items-center gap-4 min-w-[130px]">
                ...
                <Select onValueChange={selectChange}>
                  <SelectTrigger className="border-none w-16">
                    <Filter
                      size={16}
                      className={`cursor-pointer ${filter === 999 ? "" : "text-gray-950"}`}
                    />
                  </SelectTrigger>
              ...
              <TableRow key={el.id}>
                ...
                <TableCell>
                  {el.status === 3 ? (
                    <div className="flex items-center">
                      ...
                      <Button variant="link">
                        <RotateCw size={16} onClick={() => handleRetry([el])} />
                      </Button>
                    </div>
                  ) : (
                ...
                <TableCell className="text-right">
                  ...
                  {isEditable ? (
                    ...
                    <Button variant="link" className="ml-4 text-gray-400 px-2">
                      {t("delete")}
                    </Button>
                  )}
                </TableCell>
