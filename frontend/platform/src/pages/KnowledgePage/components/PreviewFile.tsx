...
export default function PreviewFile({
  ...
  const selectedChunkIndex = useKnowledgeStore(
    (state) => state.selectedChunkIndex,
  );
  const selectedChunkDistanceFactor = useKnowledgeStore(
    (state) => state.selectedChunkDistanceFactor,
  );
  ...
  const render = (type) => {
    ...
    switch (type) {
      ...
      case "txt":
        return <TxtFileViewer filePath={url} />;
      case "md":
        return <TxtFileViewer markdown filePath={url} />;
      case "html":
        return <TxtFileViewer html filePath={url} />;
      case "doc":
      case "docx":
        return <DocxPreview filePath={url} />;
      case "png":
      case "jpg":
      case "jpeg":
      case "bmp":
    ...
    >
      <div className="flex justify-center items-center relative h-10 mb-2 text-sm">
        <div className={`${labelChange ? "" : "hidden"} flex items-center`}>
          ...
          <span className="text-primary cursor-pointer" onClick={handleOvergap}>
            {t("overwriteSegment")}
          </span>
        </div>
      </div>
