...
export default function FileUploadStep1({ hidden, onNext, onSave }) {
  ...
  const handleFileChange = (files, failFiles) => {
    filesRef.current = files.map((file) => ({
      ...
      fileType: ["xlsx", "xls", "csv"].includes(
  ...
  return (
    <div
      className={`relative h-full max-w-[1200px] mx-auto flex flex-col px-10 pt-4 ${hidden ? "hidden" : ""}`}
    >
      ...
      <div className="flex justify-end gap-4 mt-8">
        ...
        <Button
          disabled={loading || !finish}
          onClick={() => onNext(filesRef.current)}
        >
          {fileCount ? <span>共{fileCount}个文件</span> : null} 下一步
        </Button>
      </div>
    </div>
  );
