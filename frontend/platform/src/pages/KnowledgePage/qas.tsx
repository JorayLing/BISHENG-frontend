...
const EditQa = forwardRef(function ({ knowlageId, onChange }, ref) {
  ...
  const handleModelGenerate = async () => {
    ...
    captureAndAlertRequestErrorHoc(
      generateSimilarQa(form.question, form.answer).then((res) => {
        setForm((prevForm) => {
          const updatedSimilarQuestions = [...prevForm.similarQuestions];
          updatedSimilarQuestions.splice(
            updatedSimilarQuestions.length - 1,
            0,
            ...res.questions,
          );
          return {
  ...
  const handleSubmit = async () => {
    ...
    const _similarQuestions = form.similarQuestions.filter(
  ...
  return (
    <Dialog open={open} onOpenChange={(bln) => (bln ? setOpen(bln) : close())}>
      <DialogContent className="sm:max-w-[625px]">
        <DialogHeader>
          <DialogTitle>
            {idRef.current ? t("updateQa") : t("createQa")}
          </DialogTitle>
        </DialogHeader>
        <div className="flex flex-col gap-4 py-2">
          ...
          <div>
            <label htmlFor="similarQuestions" className="bisheng-label">
              {t("similarQuestions")}
            </label>
            <InputList
...
export default function QasPage() {
  ...
  const handleDeleteSelected = () => {
    bsConfirm({
      ...
      onOk(next) {
        captureAndAlertRequestErrorHoc(
          deleteQa(selectedItems).then((res) => {
            reload();
          }),
        );
        next();
        ...
        <div className="flex justify-between">
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center">
              ...
              <span className="text-gray-700 text-sm font-black pl-4 dark:text-white">
                {title}
              </span>
            </div>
          </div>
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-4 items-center">
              ...
              <Button
                ...
                onClick={() => {
                  getQaFile(id).then((res) => {
                    ...
                    downloadFile(
                      checkSassUrl(fileUrl),
                      `${title} ${formatDate(new Date(), "yyyy-MM-dd")}.xlsx`,
                    );
                  });
                }}
              >
              ...
              <Button className="px-8" onClick={() => editRef.current.open()}>
                {t("createQA")}
              </Button>
            </div>
          </div>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="w-[130px] flex items-center gap-4">
                  {t("type")}
                </TableHead>
                ...
                <TableRow key={el.id}>
                  ...
                  {hasPermission ? (
                    <TableCell className="text-right">
                      <Button
                        variant="link"
                        onClick={() => editRef.current.edit(el)}
                        className="ml-4"
                      >
                        {t("update")}
                      </Button>
                    ...
                    <TableCell className="text-right">
                      <Button variant="link" disabled className="ml-4">
                        {t("update")}
                      </Button>
                      <Button
                        variant="link"
