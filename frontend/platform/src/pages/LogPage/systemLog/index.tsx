...
export default function SystemLog() {
  ...
  const {
    ...
    setPage,
    filterData,
  } = useTable({ pageSize: 20 }, (param) => getLogsApi({ ...param }));
  const init = {
    userIds: [],
    groupId: "",
    start: undefined,
  ...
  const handleActionOpen = async () => {
    setActions(
      keys.moduleId
        ? await getActionsByModuleApi(keys.moduleId)
        : await getActionsApi(),
    );
  };
  const handleSearch = () => {
    const startTime =
      keys.start && formatDate(keys.start, "yyyy-MM-dd HH:mm:ss");
    const endTime =
      keys.end &&
      formatDate(keys.end, "yyyy-MM-dd HH:mm:ss").replace(
        "00:00:00",
        "23:59:59",
      );
        ...
        <div className="flex flex-wrap gap-4">
          <div className="w-[200px] relative">
            <MultiSelect
              ...
              onSearch={(key) => {
                searchUser(key);
                selectedRef.current = keys.userIds;
              }}
          ...
          <div className="w-[180px] relative">
            <DatePicker
              ...
              onChange={(t) => setKeys({ ...keys, start: t })}
            />
          </div>
          <div className="w-[180px] relative">
            <DatePicker
          ...
          <div className="w-[180px] relative">
            <Select
              ...
              onValueChange={(value) =>
                setKeys({ ...keys, action: "", moduleId: value })
              }
            >
          ...
          <div className="w-[180px] relative">
            ...
            >
              ...
              <SelectContent>
                <SelectGroup>
                  {actions.map((a) => (
                    <SelectItem value={a.value} key={a.value}>
                      {t(a.name)}
                    </SelectItem>
                  ))}
                </SelectGroup>
        ...
        <Table className="mb-[50px]">
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="w-[150px] min-w-[100px]">
          ...
          <TableBody>
            {logs.map((log: any) => (
              <TableRow key={log.id}>
                ...
                <TableCell>
                  <div className="max-w-[200px] break-all truncate-multiline">
                    {log.operator_name}
                  </div>
                </TableCell>
...
const useUsers = () => {
  ...
  const search = (name) => {
    const newUsers = userRef.current.filter(
      (u) =>
        u.label.toLowerCase().includes(name.toLowerCase()) ||
