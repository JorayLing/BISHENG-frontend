...
export default function AppUseLog() {
  ...
  const {
    ...
    return getAuditAppListApi({
      ...
      flow_ids: param.appName?.length
        ? param.appName.map((el) => el.value)
        : undefined,
  ...
  const handleExport = async () => {
    ...
    if (!originalStart && !originalEnd) {
      ...
      adjustedStart = new Date(
        adjustedEnd.getTime() - 59 * 24 * 60 * 60 * 1000,
      ); // 最近60天
      showToast = true;
    ...
    exportCsvDataApi({
      flow_ids: filters.appName?.length
        ? filters.appName.map((el) => el.value)
        : undefined,
      ...
      .then(async (res) => {
        ...
        const handleMessage = (msg, category, id) => {
          ...
          // output
          if ("output_with_input_msg" === category)
            return `${msg.msg} :${msg.hisValue}`;
          if ("output_with_choose_msg" === category)
            return `${msg.msg} :${msg.options.find((el) => el.id === msg.hisValue)?.label}`;
        ...
        res.data.forEach((item) => {
          item.messages.forEach((msg) => {
            ...
            const usefulMsg =
              !["flow", "tool_call", "tool_result"].includes(category) &&
              message;
            usefulMsg &&
              data.push([
                item.chat_id,
      ...
      <div className="h-[calc(100vh-128px)] overflow-y-auto px-2 py-4 pb-20">
        <div className="flex flex-wrap gap-4">
          <FilterByApp
            ...
            onChange={(value) =>
              dispatch({ type: "SET_FILTER", payload: { ["appName"]: value } })
            }
          />
          <FilterByUser
            value={filters.userName}
            onChange={(value) =>
              dispatch({ type: "SET_FILTER", payload: { ["userName"]: value } })
            }
          />
          <FilterByUsergroup
            ...
            <div className="w-[200px] relative">
              <Select
                ...
                onValueChange={(value) =>
                  dispatch({
                    type: "SET_FILTER",
                    payload: { ["sensitive_status"]: value },
                  })
                }
              >
                <SelectTrigger className="w-[200px]">
          ...
          <Button
            onClick={() => {
              ...
              filterData({
                ...filters,
                dateRange: [adjustedStart, adjustedEnd],
              });
            }}
          >
            查询
          </Button>
          <Button onClick={resetClick} variant="outline">
            重置
          </Button>
              ...
              <TableRow key={el.id}>
                ...
                <TableCell className="break-all flex gap-2">
                  <div className="text-center text-xs relative">
                    ...
                    />
                    <span className="left-4 top-[-4px] break-keep">
                      {el.like_count}
                    </span>
                  </div>
                  <div className="text-center text-xs relative">
