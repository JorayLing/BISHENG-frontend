...
import {
  changeAssistantStatusApi,
  deleteAssistantApi,
} from "@/controllers/API/assistant";
import {
...
export default function apps() {
  ...
  const {
    ...
    refreshData,
    filterData,
  } = useTable<FlowType>({ pageSize: 14 }, (param) => getAppsApi(param));
  ...
  const handleCheckedChange = (checked, data) => {
    if (data.flow_type === 1) {
      return captureAndAlertRequestErrorHoc(
        updataOnlineState(data.id, data, checked).then((res) => {
          if (res) {
            refreshData((item) => item.id === data.id, {
              status: checked ? 2 : 1,
            });
          }
          return res;
      ...
      return captureAndAlertRequestErrorHoc(
        changeAssistantStatusApi(data.id, checked ? 2 : 1),
      ).then((res) => {
        if (res === null) {
          refreshData((item) => item.id === data.id, {
            status: checked ? 2 : 1,
      ...
      return captureAndAlertRequestErrorHoc(
        onlineWorkflow(data, checked ? 2 : 1),
      ).then((res) => {
        if (res) {
          refreshData((item) => item.id === data.id, {
            status: checked ? 2 : 1,
      ...
      onOk(next) {
        const promise =
          data.flow_type == 5
            ...
            : deleteFlowFromDatabase(data.id);
  ...
  const handleCreateApp = async (type, tempId = 0) => {
    if (type === AppType.SKILL) {
      ...
      captureAndAlertRequestErrorHoc(
        saveFlowToDatabase({ ...flow, id: flow.flow_id }).then((res: any) => {
        ...
        <div className="flex gap-4">
          ...
          {user.role === "admin" && (
            <Button
              ...
              onClick={() =>
                navigate(
                  `/build/temps/${tempTypeRef.current && tempTypeRef.current !== AppType.ALL ? tempTypeRef.current : AppType.FLOW}`,
                )
              }
            >
              <MoveOneIcon className="dark:text-slate-50" />
              {t("build.manageAppTemplates")}
            </Button>
          )}
        </div>
        {/* list */}
        {loading ? (
          ...
          <div className="mt-6 flex gap-2 flex-wrap pb-20 min-w-[980px]">
            <AppTempSheet
              ...
              onCustomCreate={handleCreateApp}
            >
              <CardComponent<FlowType>
                ...
                description={
                  <>
                    <p>
                      <p>{t("build.provideSceneTemplates")}</p>
                    </p>
                  </>
                }
              ></CardComponent>
              ...
              <CardComponent<FlowType>
                ...
                onSwitchClick={() => {
                  !item.write &&
                    item.status !== 2 &&
                    message({
                      description: t("build.noPermissionToPublish", {
                        type: typeCnNames[item.flow_type],
                      }),
                      variant: "warning",
                    });
                }}
                ...
                headSelecter={
                  // 技能版本
                  item.flow_type !== AppNumType.ASSISTANT ? (
                    <CardSelectVersion
                ...
                footer={
                  <Badge
                    className={`absolute py-0 px-1 right-0 bottom-0 rounded-none rounded-br-md  ${item.flow_type === AppNumType.SKILL && "bg-gray-950"} ${item.flow_type === AppNumType.ASSISTANT && "bg-[#fdb136]"}`}
                  >
