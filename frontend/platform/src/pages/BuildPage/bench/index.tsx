...
export default function index() {
  ...
  const addModel = () => {
    setFormData((prev) => ({
      ...
      models: [
        ...prev.models,
        { key: generateUUID(4), id: "", name: "", displayName: "" },
      ],
    }));
  };
        ...
        <CardContent className="pt-4 relative  ">
          <div className="w-full  max-h-[calc(100vh-180px)] overflow-y-scroll scrollbar-hide">
            <ToggleSection
              ...
              onToggle={(enabled) =>
                setFormData((prev) => ({ ...prev, menuShow: enabled }))
              }
            >
              {null}
            </ToggleSection>
            ...
            <div className="flex gap-8 mb-6">
              <IconUploadSection
                ...
                onUpload={(fileUrl, relativePath) =>
                  uploadAvator(fileUrl, "sidebar", relativePath)
                }
              />
              <IconUploadSection
                label="欢迎页面图标&对话头像"
                ...
                onUpload={(fileUrl, relativePath) =>
                  uploadAvator(fileUrl, "assistant", relativePath)
                }
              />
            </div>
            ...
            <FormInput
              ...
              maxLength={1000}
              onChange={(v) =>
                handleInputChange("functionDescription", v, 1000)
              }
            />
            ...
            <div className="mb-6">
              ...
              <FormInput
                label={
                  <Label className="bisheng-label block pt-2">
                    知识库/联网检索结果最大字符数
                  </Label>
                }
                type="number"
            ...
            >
              <FormInput
                label={
                  <Label className="bisheng-label">个人知识库搜索提示词</Label>
                }
                isTextarea
...
{question}`,
      ...
      // 检查重复（仅在非空且长度有效时检查）
        ...
        formData.models.some((m, i) => {
          if (i !== index) {
            ...
            if (
              m.displayName.trim().toLowerCase() === displayName.toLowerCase()
            ) {
              error[1] = "显示名称不能重复";
            }
            if (error[0] || error[1]) {
  ...
  const handleSave = async () => {
    ...
    captureAndAlertRequestErrorHoc(setWorkstationConfigApi(dataToSave)).then(
      (res) => {
        if (res) {
