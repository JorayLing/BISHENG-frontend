  ...
  useImperativeHandle(ref, () => ({
    ...
    open(type: AppType, tempId?: number) {
      ...
      setFormData({
        ...
        name: "",
        desc:
          AppType.ASSISTANT === type
            ? `${t("build.example", { ns: "bs" })}：
...
3. …`
  ...
  const validateField = (name: string, value: string) => {
    switch (name) {
      case "name":
        if (value.length > 50)
          return AppType.ASSISTANT === appType
            ? t("maxNameLengthAssistant")
            : t("maxNameLengthWorkflow");
        return "";
      case "desc":
        if (AppType.ASSISTANT === appType && value.length < 20)
          return t("minDescLengthAssistant");
        return "";
      default:
        return "";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (tempDataRef.current) {
      ...
      if (AppType.FLOW === appType) {
        ...
        // 使用默认模型 清空知识库和工具
        const [workflow, assistant] = await Promise.all([
          getLlmDefaultModel(),
          getAssistantModelConfig(),
        ]);
        const workflowModelId = workflow.model_id;
        const assistantModelId = assistant.llm_list.find(
          (item) => item.default,
        ).model_id;
        ...
        tempDataRef.current.data.nodes.forEach((node) => {
          if (
            ["rag", "llm", "agent", "qa_retriever"].includes(node.data.type)
          ) {
            node.data.group_params.forEach((group) =>
              group.params.forEach((param) => {
                if (param.type === "bisheng_model") {
                  ...
                  param.type === "knowledge_select_multi" &&
                  param.value.type !== "tmp"
                ) {
                  param.value.value = [];
      ...
      if (appType === AppType.ASSISTANT) {
        const res = await captureAndAlertRequestErrorHoc(
          createAssistantsApi(formData.name, formData.desc, formData.url),
        );
        if (res) {
          //@ts-ignore
        ...
        // 创建工作流
        const workflow = await captureAndAlertRequestErrorHoc(
          createWorkflowApi(formData.name, formData.desc, formData.url),
        );
        if (workflow) {
      ...
      <DialogContent className="sm:max-w-[625px] bg-background-login">
        <DialogHeader>
          <DialogTitle>
            ...
            {appType === AppType.ASSISTANT ? t("assistant") : t("workflow")}
          </DialogTitle>
        </DialogHeader>
        ...
        <DialogFooter>
          <DialogClose>
            <Button
              ...
              className="px-11"
              type="button"
              onClick={() => setFormData({ name: "", desc: "", url: "" })}
            >
              {t("cancel")}
            </Button>
          </DialogClose>
