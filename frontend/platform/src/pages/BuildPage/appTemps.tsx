...
export default function Templates() {
  ...
  const handleDragEnd = ({ source, destination }: any) => {
    ...
    captureAndAlertRequestErrorHoc(
      updateTempApi(currentItem.id, { name, description, order_num }).then(
        onChange,
      ),
    );
  };
  ...
  return (
    <div className="px-2 py-4 h-full relative">
      <div className="h-full w-full overflow-y-auto overflow-x-hidden scrollbar-hide">
        <div className="flex justify-between">
          <SelectType
            defaultValue={type}
            onChange={(v) => navigate(`/build/temps/${v}`)}
          />
          <Button size="sm" onClick={() => navigate("/build/apps")}>
            {t("returnToAppList")}
          </Button>
        </div>
                        ...
                        >
                          ...
                          <TableCell className="text-right pr-5">
                            <Button
                              ...
                              onClick={() => handleDelTemp(index, temp.id)}
                            >
                              {t("delete")}
                            </Button>
                          </TableCell>
      ...
      <div className="flex justify-between items-center absolute bottom-0 right-0 w-full py-4 pl-[16px] h-[60px] bg-background-login">
        <p className="text-gray-500 text-sm">
          {t("templateManagementDescription")}
        </p>
        <span></span>
      </div>
    </div>
