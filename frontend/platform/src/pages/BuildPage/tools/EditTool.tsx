...
>((props: any, ref) => {
  ...
  const handleTest = async () => {
    ...
    const { server_host, children, auth_method, parameter_name } =
      toolRef.current;
...
const EditTool = forwardRef((props: any, ref) => {
  ...
  const handleImportSchema = () => {
    ...
    captureAndAlertRequestErrorHoc(
      downloadToolSchema({ download_url: schemaUrl.current }),
    ).then((res) => {
      schemaUrl.current = "";
      if (!res) return;
  ...
  const handleSelectTemplate = (key = "") => {
    ...
    file_content &&
      captureAndAlertRequestErrorHoc(downloadToolSchema({ file_content })).then(
        (res) => {
          schemaUrl.current = "";
          if (!res) return;
          fromDataRef.current = { ...res, id: fromDataRef.current.id };
  ...
  const handleDelete = () => {
    bsConfirm({
      ...
      onOk(next) {
        // api
        captureAndAlertRequestErrorHoc(deleteTool(fromDataRef.current.id)).then(
          (res) => {
            if (res === false) return;
            props.onReload();
            setEditShow(false);
      ...
      <Sheet open={editShow} onOpenChange={setEditShow}>
        <SheetContent className="w-[800px] sm:max-w-[800px] p-4 bg-background-login">
              ...
              <div className="flex gap-2">
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className="dark:bg-[#282828]">
                      <Plus /> {t("tools.importFromUrl")}
                    </Button>
                  </PopoverTrigger>
                ...
                <Select
                  value="1"
                  onValueChange={(k) => handleSelectTemplate(k)}
                >
                  <SelectTrigger>
              ...
              {formState.authMethod === "apikey" && (
                <>
                  <div className="px-6 mb-4">
                    <Label
                      htmlFor="open"
                      className="bisheng-label flex items-center gap-1"
                    >
                      Auth Type
                      <QuestionTooltip
                        content={
                          <div>
                            <p>
                              Basic & Bearer：在 header 中使用 Authorization
                              传入 API key
                            </p>
          ...
          <SheetFooter className="absolute bottom-0 right-0 w-full px-6 py-4">
            ...
            <Button size="sm" className="text-[white]" onClick={handleSave}>
              {t("tools.save")}
            </Button>
          </SheetFooter>
        </SheetContent>
