...
const TestDialog = forwardRef((props, ref) => {
  ...
  const handleTest = async () => {
    ...
    if (errors.length > 0) {
      return message({
        description: `以下参数必填：${errors.join(", ")}`,
        variant: "warning",
      });
    }
    ...
    try {
      ...
      const { server_host, children, auth_method, auth_type, api_key } =
        serverRef.current;
                ...
                {Object.entries(toolData?.inputSchema?.properties || {}).map(
                  ([name, schema]) => (
                    <TableRow key={name}>
                      <TableCell>
                        {name}
                        {toolData?.inputSchema.required?.includes(name) && (
...
// 主组件  TODO(重构,状态混乱)
const McpServerEditorDialog = forwardRef(
  ({ existingNames = [], onReload }, ref) => {
      ...
      gaode: JSON.stringify(
        {
          mcpServers: {
            "amap-sse": {
              name: "高德地图",
              description:
                "提供全场景覆盖的地图服务，包括地理编码、逆地理编码、IP 定位、天气查询、骑行路径规划、步行路径规划、驾车路径规划、公交路径规划、距离测量、关键词搜索、周边搜索、详情搜索等。",
    ...
    const loadToolsFromSchema = async (schemaContent) => {
      ...
      try {
        ...
        if (tools) {
          ...
          const parsedApis = tools.children.map((item) =>
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <Button
                    variant="outline"
                    disabled={isLoading}
                    onClick={() => loadToolsFromSchema(formData.openapiSchema)}
                  >
                    刷新
                  </Button>
                </div>
                      ...
                      <TableRow key={tool.name}>
                        ...
                        <TableCell>
                          <Button
                            variant="outline"
                            onClick={() =>
                              testToolDialogRef.current.open(
                                tool,
