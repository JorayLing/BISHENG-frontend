...
export default function ToolItem({
  ...
  return (
    <AccordionItem
      key={data.id}
      value={data.id}
      className="data-[state=open]:border-2 data-[state=open]:border-primary/20 data-[state=open]:rounded-md"
    >
      <AccordionTrigger>
        <div className="group w-full flex gap-2 text-start relative pr-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="w-full text-sm font-medium leading-none flex items-center gap-2">
              {data.name}
              {["edit", "mcp"].includes(type) && data.write && (
                <div
            ...
            <div key={api.name} className="relative p-4 rounded-sm  border-t">
              ...
              {api.api_params?.length > 0 && (
                <p className="text-sm text-muted-foreground mt-2 flex gap-2">
                  <TooltipProvider>
                    <Tooltip delayDuration={100}>
                      <TooltipTrigger asChild>
                        <span className="text-primary cursor-pointer flex items-center">
                          {t("build.params")}
                      ...
                      <TooltipContent
                        side="right"
                        className="bg-gray-50 border shadow-md p-4 text-gray-950 max-w-[520px]"
                      >
                        <p className="flex gap-2 items-center">
                          <Badge>
                            {JSON.parse(api.extra)?.method || "http"}
                          </Badge>
                  ...
                  {api.api_params.map((param) => (
                    <div>
                      <span className=" rounded-xl bg-gray-200 dark:bg-background-login px-2 py-1 text-xs font-medium text-white">
                        {param.name}
                      </span>
                      {/* <span>{param.schema.type}</span> */}
                    </div>
              ...
              {select &&
                (select.some((_) => _.id === api.id) ? (
                  <Button
                    size="sm"
                    className="absolute right-4 bottom-2 h-6"
                    disabled
                  >
                    {t("build.added")}
                  </Button>
