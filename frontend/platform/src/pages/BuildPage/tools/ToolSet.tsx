...
const ToolSet = forwardRef(function ToolSet({ onChange }, ref) {
  ...
  const handleSubmit = async (formdata) => {
    await captureAndAlertRequestErrorHoc(
      updateAssistantToolApi(idRef.current, formdata),
    );
    setOpen(false);
  ...
  const renderFormContent = () => {
    switch (name) {
      ...
      case "Firecrawl":
        return (
          <CrawlerConfigForm formData={formData} onSubmit={handleSubmit} />
        );
      case "Jina AI":
        return <JinaApiKeyForm formData={formData} onSubmit={handleSubmit} />;
      case "SiliconFlow":
        return (
          <SiliconFlowApiKeyForm formData={formData} onSubmit={handleSubmit} />
        );
      case "发送邮件":
      ...
      case "天眼查":
        return (
          <TianyanchaToolForm formData={formData} onSubmit={handleSubmit} />
        );
      default:
        return null;
    }
