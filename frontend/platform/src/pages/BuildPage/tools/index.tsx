...
import {
  getAssistantMcpApi,
  getAssistantToolsApi,
  refreshAssistantMcpApi,
...
const TabTools = ({ select = null, onSelect }: TabToolsProps) => {
  ...
  const options = useMemo(() => {
    return allData.filter((el) => {
      ...
      const targetStr = `${el.name}-${el.description}-${
        el.children
          ?.map((el) => {
            // mcp 搜索包含参数名 参数描述
            const param =
      ...
      <div className="relative w-full flex h-full overflow-y-scroll scrollbar-hide bg-background-main border-t">
        <div className="relative w-fit p-6">
          ...
          <div className="mt-4">
            ...
            <div
              className={`mt-1 flex cursor-pointer items-center gap-2 rounded-md px-4 py-2 transition-all duration-200 hover:bg-muted-foreground/10 ${
                type === "mcp" && "bg-muted-foreground/10"
              }`}
              onClick={() => setType("mcp")}
            >
              <CpuIcon />
            ...
            {options.length ? (
              options.map((el) => (
                <ToolItem
                  ...
                  onSelect={onSelect}
                  onSetClick={
                    hasSet(el.name) ? () => toolsetRef.current.edit(el) : null
                  }
                  onEdit={(id) => {
                    type === "mcp"
                      ? mcpDialogRef.current.open(el)
...
const useToolType = (setType) => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      document
        .getElementById(type === "mcp" ? "create-mcptool" : "create-apitool")
        ?.click();
    }, 100);
  }, [searchParams]);
};
