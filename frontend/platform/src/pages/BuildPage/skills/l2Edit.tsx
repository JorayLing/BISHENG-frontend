...
export default function l2Edit() {
  ...
  const isParamError = (name, desc, showErrorConfirm = false) => {
    ...
    if (errorlist.length && showErrorConfirm)
      message({
        title: t("prompt"),
        variant: "error",
  ...
  const handleJumpFlow = async () => {
    ...
    // 高级配置信息有误直接跳转L3
    if (isParamError(name, description))
      return navigate("/skill/" + id, { replace: true });
    // 保存在跳
    setLoading(true);
  ...
  const handleSave = async () => {
    ...
    setTimeout(
      () => /^\/skill\/[\w\d-]+/.test(location.pathname) && navigate(-1),
      2000,
    );
  };
  ...
  const showContent = (e) => {
    ...
    target.children[0].style.transform = contentDom.clientHeight
      ? "rotate(180deg)"
      : "rotate(0deg)";
      ...
      <div className="p-6 pb-48 h-full overflow-y-auto">
        ...
        <div className="pt-6">
          ...
          <div className="w-[50%] max-w-2xl mx-auto">
            <p
              className="text-center text-gray-400 mt-4 cursor-pointer flex justify-center"
              onClick={showContent}
            >
              {t("skills.basicInfo")}
              <ChevronUp />
            ...
            <div className="w-full overflow-hidden transition-all px-1">
              ...
              <div className="mt-4">
                ...
                <Textarea
                  ...
                  className={`mt-2 ${error.desc && "border-red-400"}`}
                />
              </div>
              <div className="mt-4">
                ...
                <Textarea
                  value={guideWords}
                  onChange={(e) => setGuideWords(e.target.value)}
                  maxLength={1000}
                  id="name"
                  placeholder={t("skills.guideWords")}
                  className={`mt-2 ${error.desc && "border-red-400"}`}
                />
              </div>
            </div>
            {
              ...
              isL2 && (
                <div className="w-full mt-8">
                  <p
                    className="text-center text-gray-400 cursor-pointer flex justify-center"
                    onClick={showContent}
                  >
                  ...
                  <div className="w-full overflow-hidden transition-all px-1">
                    {flow?.data?.nodes.map(({ data }) => (
                      <div key={data.id} className="w-full">
                        ...
                        {
                          // 自定义组件
                          Object.keys(data.node.template).map(
                            (k) =>
                              data.node.template[k].l2 && (
                                <div className="w-full mt-4 px-1" key={k}>
                                  <Label htmlFor="name" className="text-right">
                                    {data.node.template[k].l2_name ||
                                      data.node.template[k].name}
                                  </Label>
                                  <L2ParameterComponent
                                    data={data}
                                    type={data.node.template[k].type}
                                    name={k}
                                  />
                                </div>
                              ),
            ...
            {appConfig.isPro && (
              <div>
                <p
                  className="text-center text-gray-400 mt-8 cursor-pointer flex justify-center"
                  onClick={showContent}
                >
                ...
                <div className="w-full overflow-hidden transition-all px-1">
                  <FlowSetting
                    id={id}
                    type={2}
                    isOnline={nextFlow?.status === 2}
                    onSubTask={(fn) => (flowSettingSaveRef.current = fn)}
                  />
                </div>
              </div>
            )}
      ...
      <div className="absolute flex z-30 bottom-0 w-[calc(100vw-200px)] py-8 mr-5 justify-center bg-background-login">
        {isL2 ? (
          <div className="flex gap-4 w-[50%]">
            <Button
              disabled={loading}
              className="extra-side-bar-save-disable w-[70%]"
              onClick={handleSave}
            >
              {t("save")}
            </Button>
          ...
          <div className="flex justify-center w-[50%]">
            <Button
              disabled={loading}
              className="extra-side-bar-save-disable w-[50%]"
