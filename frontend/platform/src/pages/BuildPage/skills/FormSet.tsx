...
export default forwardRef(function FormSet({ id, vid }: any, ref) {
  ...
  useEffect(() => {
    getVariablesApi({ version_id: vid, flow_id: id }).then((res) =>
      setItems(res),
    );
  }, []);
                      ...
                      >
                        <div className="flex justify-between">
                          <div className="flex gap-2 relative items-center">
                            ...
                            <label className="font-medium text-sm max-w-[200px] truncate">
                              {item.name}
                            </label>
                          ...
                          <div className="flex items-center gap-2">
                            <label className="text-xs">
                              {t("report.requiredLabel")}
                            </label>
                            <Switch
                              checked={item.type === "file" || item.required}
                              onCheckedChange={(e) => {
                                item.type !== "file" &&
                                  setItems((old) => {
                        ...
                        <div className="mt-2">
                          {item.type === "text" && (
                            <div className="cursor-pointer h-10 border rounded-sm"></div>
                          )}
                          {item.type === "select" && (
                            <div className="cursor-pointer h-10 border rounded-sm flex items-center justify-end px-2">
                              <ChevronsUpDown className="dropdown-component-arrow-color" />
                            </div>
                          )}
                          {item.type === "file" && (
                            <div className="cursor-pointer flex h-16 justify-center items-center border rounded-sm">
                              <FolderUp />
                            </div>
                          )}
                        </div>
