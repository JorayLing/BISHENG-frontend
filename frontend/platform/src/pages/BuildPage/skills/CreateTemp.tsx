...
export default function CreateTemp({ flow, open, type, setOpen, onCreated }) {
  ...
  const handleSubmit = () => {
    ...
    if (!description && type === AppType.ASSISTANT)
      errorlist.push(t("build.addDescription", { labelName }));
    if (description.length > 200)
      errorlist.push(t("build.descriptionTooLong", { labelName }));
    if (errorlist.length)
      message({
        variant: "error",
    ...
    captureAndAlertRequestErrorHoc(
      createTempApi({ ...data, flow_id: flow.id }, type).then((res) => {
        setOpen(false);
        message({
          variant: "success",
            ...
            <label htmlFor="name" className="bisheng-label">
              {type === AppType.SKILL
                ...
                : type === AppType.ASSISTANT
                  ? t("build.assistantName")
                  : t("build.workFlowName")}
            </label>
            <Input
              name="name"
              className="mt-2"
