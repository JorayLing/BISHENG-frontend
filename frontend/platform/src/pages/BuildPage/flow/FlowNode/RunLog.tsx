...
export default function RunLog({ node, children }) {
  ...
  useEffect(() => {
    const buildData = (data) => {
      if (data) {
        ...
        let hasKeys = [];
        const isFormInputNode =
          node.type === "input" && node.tab.value === "form_input";
        ...
        node.group_params.forEach((group) => {
          group.params.forEach((param) => {
            ...
            if (
              ...
              param.value.some((el) => newData.has(el.key))
            ) {
        ...
        return Array.from(newData.entries()).map(([key, value]) => ({
          label:
            ["file", "variable"].includes(value.type) &&
    ...
    return (
      <div className="bisheng-node-top flex justify-between bg-[#E6FBF1] dark:bg-[#303134]">
        <div className="flex items-center gap-2 text-sm">
          <div className="rounded-full w-4 h-4 bg-[#00C78C] text-gray-50 flex items-center justify-center">
            <Check size={14} />
          </div>
  ...
  return (
    <div className="bisheng-node-top flex justify-between bg-[#FCEAEA] dark:bg-[#303134]">
      <div className="flex items-center gap-2 text-sm">
        <div className="rounded-full w-4 h-4 bg-[#F04438] text-gray-50 flex items-center justify-center">
          <X size={14} />
        </div>
