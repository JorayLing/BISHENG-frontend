...
const isMatch = (obj, expression) => {
  ...
  if (obj.key === "form_input") {
    expression =
      "value.filter(el => el.type !== 'file').map(el => ({ label: el.key, value: el.key }))";
  }
...
const getSpecialVar = (obj, type, onlyImg = false) => {
  switch (type) {
    ...
    case "item:input_list":
      ...
      param.value = param.value.map((item) => ({
        label: item.value,
        value: item.key,
      }));
...
const SelectVar = forwardRef(
  (
      ...
      item.data.forEach((group) => {
        group.params.forEach((param) => {
          ...
          if (param.global.indexOf("code") === 0) {
            ...
            (param.global === "key" && nodeId !== item.id) ||
    ...
    function processInputNode(inputNodeData, findInputFileOnly) {
      ...
      // 根据文件类型过滤参数
      if (acceptType === "file") {
        groupParam.params = groupParam.params.filter(
          (param) => param.key !== "dialog_image_files",
        );
      }
      // 如果只需要文件输入参数
      ...
      return nodeTemps.reduce((acc, itemL1) => {
        ...
        itemL1.data.forEach((itemL2) => {
          ...
          if (itemL2.value === "preset_question") {
            // 处理三级菜单
            const { keys: nestedKeys, checkedCount: checkedCountL2 } =
              handleNestedItems(keyL2, itemL2.param.value, valueSet);
      ...
      if (variable && variable.value !== "preset_question") {
        ...
        currentNode.data?.map((item) =>
          item.param
            ...
            : variable?.value !== "preset_question" &&
    ...
    return (
      <Select open={open} onOpenChange={setOpen}>
        <SelectTrigger
          ...
          showIcon={false}
          className={cname(
            "group shrink min-w-0 p-0 h-auto data-[placeholder]:text-inherit border-none bg-transparent shadow-none outline-none focus:shadow-none focus:outline-none focus:ring-0",
            className,
          )}
        >
          {children}
        ...
        <SelectContent position="popper" avoidCollisions={false}>
          <div className="flex max-h-[360px] ">
            ...
            <div className="w-36 min-w-36 border-l first:border-none overflow-y-auto  scrollbar-hide">
              {nodeTemps.map((item) => (
                ...
                >
                  {onCheck && (
                    <Checkbox
                      checked={checkKeys[item.id]}
                      onCheckedChange={(bln) => handleCheckClick(bln, item.id)}
                      className="mr-1"
                    />
                  )}
                  {item.icon}
                  <span className="w-28 overflow-hidden text-ellipsis ml-2">
                    {item.name}
                  </span>
                ...
                {vars.map((v) => (
                  ...
                  >
                    {onCheck && (
                      <Checkbox
                        checked={
                          checkKeys[`${currentMenuRef.current.id}.${v.value}`]
                        }
                        className="mr-1"
                        onCheckedChange={(bln) =>
                          handleCheckClick(bln, currentMenuRef.current.id, v)
                        }
            ...
            {!!questions.length && (
              <div className="w-44 min-w-36 border-l first:border-none">
                {questions.map((q) => (
                  <div
                  ...
                  >
                    {onCheck && (
                      <Checkbox
                        checked={
                          checkKeys[`${currentMenuRef.current.id}.${q.value}`]
                        }
                        className="mr-1"
                        onCheckedChange={(bln) =>
                          handleCheckClick(bln, currentMenuRef.current.id, q)
                        }
                        onClick={(e) => e.stopPropagation()}
                      />
                    )}
