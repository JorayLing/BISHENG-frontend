...
export default function VarTextareaUploadItem({
  nodeId,
  data,
  onChange,
  onValidate,
  ...
  return (
    <div className="node-item mb-4 nodrag" data-key={data.key}>
      <div className="flex justify-between items-center">
        <Label className="flex items-center bisheng-label">消息内容变量</Label>
...
export const useFileUpload = (_files, onFilesChange) => {
  ...
  const handleFileUpload = () => {
    ...
    input.type = "file";
    input.accept =
      "image/*,application/pdf,text/plain,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.openxmlformats-officedocument.presentationml.presentation,text/markdown,text/html,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/msword,application/vnd.ms-powerpoint,.png,.jpg,.jpeg"; // Restrict to images
      ...
      if (file) {
        uploadFileWithProgress(
          ...
          .then((res) => {
            ...
            const newFiles = [
              ...files,
              { name: file.name, path: res.relative_path },
            ];
            setFiles(newFiles);
