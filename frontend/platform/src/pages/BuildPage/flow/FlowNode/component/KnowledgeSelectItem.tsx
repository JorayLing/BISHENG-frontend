...
export default function KnowledgeSelectItem({
  data,
  nodeId,
  onChange,
  onVarEvent,
  ...
  const handleSelect = (vals) => {
    const resVals =
      currentTabRef.current === tabType ? vals : [vals[vals.length - 1]];
    setValue(resVals);
    onChange({
      type: tabType,
      value: resVals.map((el) => ({
        // 夸类型先清空value
        key: el.value,
        label: el.label,
      })),
  ...
  useEffect(() => {
    ...
    onValidate((config) => {
      ...
      if (
        data.value.value.some((item) =>
          /input_[a-zA-Z0-9]+\.file/.test(item.key),
        )
      ) {
        return "input_file";
      }
  ...
  const validateVarAvailable = async (config) => {
    ...
    if (typeof value[0].value === "number") {
      const effectiveKnowledges = await getKnowledgeDetailApi(
