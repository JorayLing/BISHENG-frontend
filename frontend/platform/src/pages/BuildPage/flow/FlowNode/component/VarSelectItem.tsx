...
export default function VarSelectItem({
  nodeId,
  data,
  onChange,
  onOutPutChange,
  ...
  const handleVarChange = useCallback(
    (checked, items) => {
      const newValues = value.filter(
        (el) =>
          !items.some(
            ({ node, variable }) => `${node.id}.${variable.value}` === el,
          ),
      );
...
// 单选
export function VarSelectSingleItem({
  nodeId,
  data,
  onChange,
  onValidate,
        ...
        >
          <div className="flex flex-wrap">
            {value ? (
              ...
              <span className="text-gray-400">{data.placeholder}</span>
            )}
          </div>
          <ChevronDown className="h-5 w-5 min-w-5 opacity-80 group-data-[state=open]:rotate-180" />
        </div>
