...
const DragOptions = forwardRef(
  (
    ...
    return (
      <div
        ref={ref}
        className={`${scroll && "nowheel overflow-y-auto max-h-80"} mt-2`}
      >
                      ...
                      >
                        <div className="group w-full flex items-center rounded-md border border-input bg-search-input shadow-sm">
                          <div
                            ...
                            className="flex flex-col justify-center border-r px-1"
                          >
    ...
    <>
      <div className="flex-1">
        {editing ? (
          ...
          <div className="h-9 leading-9 w-[200px] px-2 text-sm text-[#111] dark:text-gray-50 cursor-not-allowed opacity-50 truncate">
            {item.text}
          </div>
        )}
      </div>
      <div className="flex gap-1 items-center pr-2">
        <span className="text-xs text-muted-foreground group-hover:hidden">
          {itemNames[item.type] || item.type}
        </span>
        {(onEditClick || edit) && (
          <Edit
            size={14}
            onClick={handleEdit}
            className="cursor-pointer text-muted-foreground hover:text-foreground hidden group-hover:block"
          />
        )}
        {count > 1 && (
          <Trash2
