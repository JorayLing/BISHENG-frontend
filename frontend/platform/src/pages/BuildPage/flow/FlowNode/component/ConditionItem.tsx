    ...
    <div className="flex gap-1 items-center mb-1 hover-reveal">
      ...
      <SelectVar
        ...
        onSelect={(E, v) => {
          onUpdateItem(index, {
            ...item,
            left_label: `${E.name}/${v.label}`,
            left_var: `${E.id}.${v.value}`,
          });
        }}
      >
        <Tip content={item.left_label} side="top">
          <div
            className={`${
              ((required && !item.left_label) || leftError) && "border-red-500"
      ...
      <Select
        ...
        onValueChange={handleCompTypeChange}
      >
        <SelectTrigger
          className={`max-w-32 w-24 h-8 ${required && !item.comparison_operation && "border-red-500"}`}
        >
          ...
          {item.right_value_type === "ref" ? (
            <SelectVar
              ...
              onSelect={(E, v) => {
                onUpdateItem(index, {
                  ...item,
                  right_label: `${E.name}/${v.label}`,
                  right_value: `${E.id}.${v.value}`,
                });
              }}
            >
              <Tip content={item.right_label} side="top">
                <div
                  className={`${
                    ((required && !item.right_label) || rightError) &&
                    "border-red-500"
                  } no-drag nowheel group flex h-8 w-full items-center justify-between whitespace-nowrap rounded-md border border-input bg-search-input px-3 py-1 text-sm shadow-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1 data-[placeholder]:text-gray-400`}
                >
...
export default function ConditionItem({
  ...
  const handleAddCondition = () => {
    ...
    setValue((val) => {
      const newVal = [
        ...val,
        { id: generateUUID(8), operator: "and", conditions: [] },
      ];
      onChange(newVal);
      return newVal;
    });
      ...
      {value.map((val, vindex) => (
        <div className="relative group" key={val.id}>
          <div className="flex justify-between items-center">
            <p className="mt-2 mb-3 text-sm font-bold">{t("if")}</p>{" "}
            {/* 如果 */}
            {value.length > 1 && (
              <Trash2
                size={14}
