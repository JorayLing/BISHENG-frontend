...
function Form({
  ...
  useEffect(() => {
    ...
    const fileOtions = existingOptions?.filter(
      (opt) => opt.type === FormType.File,
    );
    ...
    if (formData.formType === FormType.File) {
      ...
      if (!formData.filecontent.trim()) {
        ...
        existingOptions?.some(
          (opt) =>
            opt.type === "file" && opt.file_content === formData.filecontent,
        ) &&
      ...
      if (!formData.filepath.trim()) {
        ...
        existingOptions?.some(
          (opt) => opt.type === "file" && opt.file_path === formData.filepath,
        ) &&
      ...
      if (formData.fileType !== "file") {
        const _error = validateImageFileVariableName(
          formData.imageFile,
          existingOptions,
        );
        if (_error) {
  ...
  const validateImageFileVariableName = (varName, existingOptions) => {
    ...
    if (
      existingOptions?.some(
        (opt) => opt.type === "file" && opt.image_file === formData.imageFile,
      ) &&
  ...
  useEffect(() => {
    if (
      ...
      formData.formType === FormType.Text
    ) {
      checkVarFuncRef.current();
    }
  ...
  const InputForm = (
    <div className="space-y-4">
      ...
      <div>
        ...
        />
        {errors.variableName && (
          <p className="text-red-500 text-sm">{errors.variableName}</p>
        )}
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center space-x-2">
        ...
        <Switch
          ...
          onCheckedChange={(checked) =>
            setFormData({ ...formData, isMultiple: checked })
          }
        />
      </div>
      <FileTypeSelect
      ...
      <div>
        <Label className="flex items-center bisheng-label">
          ...
          <QuestionTooltip
            content={
              "文件将会上传到以此命名的临时知识库中，可在文档知识库问答、助手等节点中使用"
            }
          />
        </Label>
        <Input
      ...
      <div>
        <Label className="flex items-center bisheng-label">
          ...
          <QuestionTooltip
            content={
              "文件解析结果全文将会存储在此变量中，使用时请注意可能会超出模型上下文长度"
            }
          />
        </Label>
        <Input
          ...
          onChange={(e) =>
            setFormData({ ...formData, filecontent: e.target.value })
          }
        />
        {errors.filecontent && (
          <p className="text-red-500 text-sm">{errors.filecontent}</p>
        )}
      </div>
      <InputItem
        type="number"
        ...
        onChange={(fileContentSize) =>
          setFormData({ ...formData, fileContentSize })
        }
      />
      <div>
        <Label className="flex items-center bisheng-label">
          文件路径变量名称
          <QuestionTooltip
            content={"文件路径将会存储在此变量中，后续可在代码节点中使用"}
          />
        </Label>
        <Input
        ...
        />
        {errors.filepath && (
          <p className="text-red-500 text-sm">{errors.filepath}</p>
        )}
      </div>
      {formData.fileType !== "file" && (
        <div>
          <Label className="flex items-center bisheng-label">
            ...
            <QuestionTooltip
              content={
                "提取上传文件中的图片文件，当助手或大模型节点使用多模态大模型时，可传入此图片。"
              }
            />
          </Label>
          <Input
            ...
            onChange={(e) =>
              setFormData({ ...formData, imageFile: e.target.value })
            }
          />
          {errors.imageFile && (
...
export default function InputFormItem({
  ...
  const options = useMemo(() => {
    ...
    return _options.map((el) => {
      ...
      return {
        ...
        text:
          el.type === "file"
            ? `${el.value}(${el.key},${el.file_content},${el.file_path})`
            : `${el.value}(${el.key})`,
        type: el.type,
      };
    });
  ...
  return (
    <div className="node-item mb-4 nodrag" data-key={data.key}>
      ...
      {error && (
        <p className="text-red-500 text-sm">{t("atLeastOneFormItem")}</p>
      )}
        ...
        <DialogContent>
          ...
          {isOpen && (
            <Form
              ...
              nodeData={data}
              initialData={
                editKey ? data.value.find((el) => el.key === editKey) : null
              } // 如果是编辑模式，传入当前表单数据
