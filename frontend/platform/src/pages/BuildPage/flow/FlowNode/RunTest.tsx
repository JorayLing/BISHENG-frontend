...
export const ResultText = ({ title, value }: { title: string; value: any }) => {
  ...
  return (
    <div
      className="nowheel nodrag mb-2 rounded-md border bg-search-input text-sm shadow-sm"
    ...
    >
      ...
      <textarea
        defaultValue={text}
        readOnly
        className="w-full min-h-28 p-2 block text-muted-foreground bg-gray-100 dark:bg-black outline-none"
      />
    </div>
  );
};
...
export const RunTest = forwardRef((props, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    run: (node: WorkflowNode) => {
            ...
            if (param.type === "var_textarea") {
              ...
              allVarInput = [
                appendAutoFillin({
                  key: param.value,
                  required: false,
                  label: param.varZh?.[param.value] || param.value,
                  value: "",
                }),
              ];
      ...
      // 去重
      setInputs((prev) => [
        ...new Map(prev.map((item) => [item.key, item])).values(),
      ]);
    },
  }));
    ...
    node.group_params.forEach((group) => {
      group.params.forEach((param) => {
        if (
          ...
          param.value.some((el) => newData[el.key])
        ) {
          // 尝试去value中匹配
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        <div className="px-2 pt-2 pb-10 h-[calc(100vh-40px)] overflow-y-auto bg-[#fff] dark:bg-[#303134]">
          ...
          {results.length !== 0 && (
            <p className="mt-2 mb-3 text-sm font-bold">{t("runResults")}</p>
          )}
          {results.length > 1 && (
            <div className="mb-2">
              <Select
