...
export const CustomHandle = ({
  ...
  if (isLeft) {
    return (
      <div className={cname("absolute top-[58px] -left-[16px]", className)}>
        <Handle
          id={id || "left_handle"}
          type="target"
  ...
  return (
    <div className={cname("absolute top-[58px] right-[-16px]", className)}>
      <Handle
        id={id || "right_handle"}
        type="source"
      ...
      {openRight && (
        ...
        >
          <CardContent className="min-w-56 pointer-events-auto px-0">
            <Sidebar dropdown onClick={handleOptionClick}></Sidebar>
          </CardContent>
        </Card>
      )}
    </div>
...
function CustomNode({
  ...
  const [currentTab, setCurrentTab] = useState<undefined | string>(
  ...
  const { paramValidateEntities, varValidateEntities, validateAll } =
    useEventMaster(node, setNodeError);
  ...
  const [expend, setExpend] = useState(
    node.expand === undefined ? true : node.expand,
  );
  ...
  return (
    ...
    >
      ...
      <div
        className={cname(
          `bisheng-node hover:border-primary/10 ${node.type === "condition" ? "w-auto min-w-80" : ""} ${selected ? "border-primary/10" : " border-transparent"}`,
          nodeError && "border-red-500",
        )}
        data-id={node.id}
      >
        {/* top */}
        ...
        <div className="-nowheel bg-[#F7F8FB] dark:bg-background pb-5 rounded-b-[20px]">
          <div
            className={
              expend || ["output", "condition", "end"].includes(node.type)
                ? ``
                : "h-0 overflow-hidden"
            }
          >
            ...
            {node.group_params.map((group) => (
              <ParameterGroup
                ...
                onStatusChange={(key, obj) =>
                  (paramValidateEntities.current[key] = obj)
                }
                onVarEvent={(key, obj) =>
                  (varValidateEntities.current[key] = obj)
                }
              />
            ))}
          </div>
        ...
        {!["condition", "output", "end"].includes(node.type) && (
          <CustomHandle node={node} />
        )}
      </div>
...
const useEventMaster = (node, setNodeError) => {
  ...
  const validateAll = async (config) => {
    ...
    // var
    const promises = Object.keys(varValidateEntities.current).map(
      async (key) => {
