...
export default function NodeLogo({ type, className = "", colorStr = "" }) {
  ...
  if (type === "tool") {
    ...
    const _colorKey =
      keys[parseInt(colorStr.charCodeAt(0) + "", 16) % keys.length];
    return (
      <div
        className={cname(
          `${Colors[_colorKey]} p-[5px] rounded-md dark:text-gray-600`,
          className,
        )}
      >
        <Hammer size={14} />
  ...
  return (
    <div
      className={cname(
        `${color} dark:text-gray-600 ${["input", "output"].includes(type) && "text-[#fff] dark:text-[#fff]"} p-[5px] rounded-md`,
        className,
      )}
    >
      <IconComp size={14} />
