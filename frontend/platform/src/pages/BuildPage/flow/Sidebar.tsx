          ...
          <Tooltip key={el.name}>
            <TooltipTrigger className="block w-full">
              <div
                ...
                onMouseEnter={(event) => {
                  if (
                    !event.currentTarget.classList.contains("border-gray-200")
                  ) {
                ...
                onDragEnd={(event) => {
                  document.body.removeChild(
                    document.getElementsByClassName("cursor-dragging")[0],
                  );
                }}
                onClick={() => {
                  if (!dropdown) return;
...
export default function Sidebar({
  ...
  const nodeTemps = useMemo(() => {
    ...
    return tempData.reduce((list, temp) => {
      ...
      temp.type === "start"
        ? onInitStartNode(cloneDeep(temp))
        : list.push(newNode);
      return list;
    }, []);
  ...
  return (
    <div
      className={`${dropdown ? "relative" : "absolute"} max-w-56 z-40 h-full transition-transform ${expand ? "p-2" : "py-2 translate-x-[-200px]"}`}
    >
      <div className="bg-background rounded-2xl shadow-md h-full p-2">
        ...
        >
          <div className="flex gap-1">
            ...
            {!dropdown && (
              <Button
                size="icon"
                variant="secondary"
                className={`${expand ? " right-[-30px]" : "right-[-46px]"} absolute bg-[#fff] dark:bg-gray-950 top-2 rounded-full size-8`}
                  ...
                  <TooltipTrigger className="block w-full">
                    <div
                      ...
                      onDragStart={(event) => {
                        onDragStart(event, {
                          type: item.type,
                          node: tempData.find((tmp) => tmp.type === item.type),
                        });
                      }}
                      onDragEnd={(event) => {
                        document.body.removeChild(
          ...
          <TabsContent
            value="tool"
            className="nowheel overflow-y-auto h-[calc(100vh-10rem)] max-w-44 relative"
          >
                ...
                {toolTemps.map((temp, index) => (
                  <ToolItem
                    ...
                    onClick={(type, tool_key) => {
                      let node = null;
                      toolTemps.some((temp) => {
                        node = temp.children.find(
                          (node) => node.tool_key === tool_key,
                        );
                        return node;
                      });
                      onClick({ type, node });
