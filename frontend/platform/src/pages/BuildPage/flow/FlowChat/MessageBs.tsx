...
export default function MessageBs({
  ...
  const message = useMemo(() => {
    const msg =
      typeof data.message === "string" ? data.message : data.message.msg;
  ...
  return (
    <div className="flex w-full">
      <div className="w-fit group max-w-[90%]">
        <ReasoningLog
          loading={!data.end && data.reasoning_log}
          msg={data.reasoning_log}
        />
          ...
          <>
            <div className="flex justify-between items-center mb-1">
              ...
              <div className={`text-right group-hover:opacity-100 opacity-0`}>
                <span className="text-slate-400 text-sm">
                  {formatStrTime(data.create_time, "MM 月 dd 日 HH:mm")}
                </span>
              </div>
            </div>
            <div className="min-h-8 px-6 py-4 rounded-2xl bg-[#F5F6F8] dark:bg-[#313336]">
              <div className="flex gap-2">
                {logo ? (
                  <div className="max-w-6 min-w-6 max-h-6 rounded-full overflow-hidden">
                    <img className="w-6 h-6" src={logo} />
                  </div>
                ) : (
                  <div
                    className="w-6 h-6 min-w-6 flex justify-center items-center rounded-full"
