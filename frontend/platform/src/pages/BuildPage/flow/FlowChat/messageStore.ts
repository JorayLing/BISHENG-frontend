...
type Actions = {
  loadHistoryMsg: (
    flowid: string,
    chatId: string,
    data: { appendHistory: boolean; lastMsg: string },
  ) => Promise<void>;
...
const handleHistoryMsg = (data: any[]): ChatMessageType[] => {
  const correctedJsonString = (str: string) =>
    str
  ...
  return data
    ...
    .map((item) => {
      let { message, files, is_bot, intermediate_steps, category, ...other } =
        item;
      try {
        message =
          message && message[0] === "{" ? JSON.parse(message) : message || "";
      } catch (e) {
        // 未考虑的情况暂不处理
      ...
      return {
        ...
        category,
        chatKey:
          typeof message === "string" ? undefined : Object.keys(message)[0],
        end: true,
...
export const useMessageStore = create<State & Actions>((set, get) => ({
  ...
  createWsMsg(data) {
    ...
    set((state) => {
      ...
      const messageId =
        message_id || (category === "guide_word" ? generateUUID(4) : ""); // 后端没给,临时生成一个
      newChat = newChat.filter(
        (item) => !(item.message_id === message_id && item.his),
      );
      newChat.push({
        category,
        flow_id,
  ...
  streamWsMsg(data) {
    ...
    const currentMessageIndex = messages.findIndex(
      (msg) => msg.message_id === unique_id + output_key,
    );
    const currentMsg = messages[currentMessageIndex];
    if (!currentMsg)
      return get().createWsMsg({
        ...data,
        message: data.message.msg,
        reasoning_log: reasoning_content || "",
        message_id: unique_id + output_key,
      });
    // append
    const newCurrentMessage = {
      ...
      message:
        data.type === "end"
          ? data.message.msg
          : currentMsg.message + data.message.msg,
      reasoning_log: reasoning_content
  ...
  insetNodeRun(data) {
    ...
    set((state) => {
      ...
      if (type === "end") {
        return {
          messages: newChat.filter(
            (msg) => msg.message_id !== message.unique_id,
          ),
        };
      }
      newChat.push({
  ...
  async loadMoreHistoryMsg(flowid, appendHistory) {
    ...
    if (msgs.length) {
      set({
        [appendHistory ? "messages" : "hisMessages"]: [
          ...msgs.reverse(),
          ...prevMsgs,
        ],
      });
