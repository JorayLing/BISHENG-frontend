...
export default function ChatInput({
  ...
  useEffect(() => {
    ...
    createWebSocket().then(() => {
      ...
      const wsMsg = onBeforSend(
        (messages.length === 0 && hisMessages.length === 0) ||
  ...
  const handleSendClick = async () => {
    ...
    // setFormShow(false)
    const [filePath, fileNames] = getFileIds().reduce(
      (acc, cur) => {
        acc[0].push(cur.path);
        acc[1].push(cur.name);
        return acc;
  ...
  const handleWsMessage = (data) => {
    if (data.category === "error") {
      ...
      // 记录
      const errorMsg =
        code == 500 ? message : t(`errors.${code}`, { type: message });
      addNotification({
        type: "error",
        title: "运行异常",
      ...
      if (data.type === "end" && data.message.input_data) {
        setRunCache(data.message.node_id, data.message.input_data);
      }
      insetNodeRun(data);
      ...
      if (input_schema.tab === "dialog_input") {
        const schemaItem = input_schema.value?.find(
          (el) => el.key === "dialog_file_accept",
        );
        ...
        if (fileAccept === "image") {
          ...
          setAccepts(
            FileTypes.IMAGE.join(",") + "," + FileTypes.FILE.join(","),
          );
        }
      }
      // 待用户输入
      input_schema.tab === "form_input"
        ? setInputForm(input_schema)
        : setInputLock({ locked: false, reason: "" });
      setTimeout(() => {
      ...
      return questionsRef.current.updateQuestions(
        data.message.guide_question.filter((q) => q),
      );
  ...
  useEffect(() => {
    const handleCustomEvent = (e) => {
      if (!showWhenLocked && inputLock.locked)
        return console.error("弹窗已锁定，消息无法发送");
  ...
  const handleRestartClick = () => {
    ...
    setInputForm(null);
    setRestarted(true);
    const chatId = currentChatIdRef.current.startsWith("test")
      ? ""
      : currentChatIdRef.current;
  ...
  // 文件上传状态
  const { fileUploading, getFileIds, loadingChange } = useFileLoading(
    inputLock.locked,
  );

  return (
    <div className="absolute bottom-0 w-full pt-1 bg-[#fff] dark:bg-[#1B1B1B]">
      <div className={`relative pr-4 ${clear && "pl-9"}`}>
        ...
        <div className="flex absolute left-3 top-4 z-10">
          {form && (
            ...
            >
              <FormIcon
                className={
                  !showWhenLocked && inputLock.locked
        ...
        <div className="flex gap-2 absolute right-7 top-4 z-10">
          ...
          >
            <SendIcon
              className={`${inputLock.locked || fileUploading ? "text-muted-foreground" : "text-foreground"}`}
            />
          </div>
        </div>
        {/* stop & 重置 */}
        ...
        <Textarea
          ...
          className={
            "resize-none py-4 pr-10 text-md min-h-6 max-h-[200px] scrollbar-hide dark:bg-[#131415] text-gray-800" +
            (form && " pl-10")
          }
          onKeyDown={(event) => {
      ...
      <p className="text-center text-sm pt-2 pb-4 text-gray-400">
