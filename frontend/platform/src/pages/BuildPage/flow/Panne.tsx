...
export default function Panne({
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      ></Header>
      <div
        className={`flex-1 min-h-0 overflow-hidden ${showApiPage ? "hidden" : ""} relative`}
      >
        <Sidebar
          onInitStartNode={(node) => {
            // start node
                ...
                setNodes([
                  {
                    id: nodeId,
                    type: "flowNode",
                    position: { x: window.innerWidth * 0.4, y: 20 },
                    data: node,
                  },
                ]);
              }, 500); // after init
...
const useFlow = (_reactFlowInstance, data, takeSnapshot) => {
  ...
  // 绑定快捷键
  const { keyBoardPanneRef, setLastSelection } = useKeyBoard(
    _reactFlowInstance,
    reactFlowWrapper,
    setNodes,
    setEdges,
  ...
  const onEdgeSelect = (obj) => {
    ...
    setEdges((eds) => {
      ...
      const leftEdge = {
        ...
        selected: false,
        target: nodeId,
        id: `xy-edge__${edge.source}${edge.sourceHandle}-${nodeId}${edge.targetHandle}`,
      };
      const rightEdge = {
        ...edge,
        selected: false,
        ...
        id: `xy-edge__${nodeId}right_handle-${edge.target}${edge.targetHandle}`,
      };

      return eds.filter((el) => el.id !== edgeId).concat(leftEdge, rightEdge);
    });
  };
  ...
  useEffect(() => {
    ...
    const handleNodeDelete = (event) => {
      ...
      setEdges((edges) =>
        edges.filter((ns) => ns.source !== nodeId && ns.target !== nodeId),
      );
    };
      ...
      const newNodes = await Promise.all(
        nodeIds.map(async (nodeId) => {
          ...
          const data = JSON.parse(
            JSON.stringify(node.data).replaceAll(nodeId, newNodeId),
          );
          // 复制报告节点中报告模板
  ...
  const handleAddNote = () => {
    ...
    const reactflowBounds = reactFlowWrapper.current.getBoundingClientRect();
    const pos = _reactFlowInstance.screenToFlowPosition({
      x: reactflowBounds.width * 0.2,
      y: reactflowBounds.height * 0.9,
    });
    const position = calculatePosition(nodes, {
      x: pos.x + 50,
      y: pos.y + 50,
...
const useKeyBoard = (_reactFlowInstance, reactFlowWrapper) => {
  ...
  useCopyPasteNode(
    keyBoardPanneRef.current,
    lastSelection,
    (newSelectNode, position) => {
      if (newSelectNode.nodes.some((node) => node.data.type === "start"))
        return;
      ...
      setNodes((nds) => {
        ...
        const newNodes = newSelectNode.nodes.map((node) => {
          ...
          const newNode = JSON.parse(
            JSON.stringify(node).replaceAll(node.id, nodeId),
          );
          newNode.id = nodeId;
    ...
    (selectNode) => {
      ...
      setNodes((nodes) =>
        nodes.filter((n) => !targetNodes.some((el) => el.id === n.id)),
      );
      setEdges((edges) =>
        edges.filter((ns) => !targetEdges.some((el) => el.id === ns.id)),
      );
    },
