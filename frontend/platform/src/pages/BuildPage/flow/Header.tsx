...
const Header = ({
  ...
  const { returnPage, blocker, hasChanged } = useBeforeUnload(
    flow,
    nodes,
    preFlow,
    onPreFlowChange,
  ...
  return (
    <header className="flex justify-between items-center p-4 py-2 bisheng-bg border-b">
      {loading && (
        <div className=" fixed left-0 top-0 w-full h-screen bg-background/60 z-50 flex items-center justify-center">
      ...
      <div className="flex items-center">
        <Button
          variant="outline"
          size="icon"
          className={`${!dark && "bg-[#fff]"} size-8`}
          onClick={returnPage}
        >
          <ChevronLeft />
        </Button>
        <div className="flex items-center ml-5">
          <TitleLogo url={flow.logo} id={2} className="">
            <AssistantIcon />
          </TitleLogo>
          <div className="pl-3">
            <h1 className="font-medium text-sm flex gap-2">
              ...
              <Button
                ...
                className="size-6"
                onClick={() =>
                  updateAppModalRef.current?.edit(AppType.FLOW, flow)
                }
              >
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              <Badge
                variant="gray"
                className="font-light dark:bg-gray-950 dark:text-gray-400"
              >
                <ShieldCheck size={14} />
                {t("currentVersion", { versionName: version?.name })}
              </Badge>
            </p>
          </div>
        </div>
      ...
      <div>
        <Button
          variant="secondary"
          className={`${tabType === "edit" ? 'bg-[#fff] dark:bg-gray-950 hover:bg-[#fff]/70 text-primary h-8"' : ""} h-8`}
          onClick={() => {
            setTabType("edit");
            onTabChange("edit");
          }}
        ...
        <Button
          variant="secondary"
          className={`${tabType === "api" ? 'bg-[#fff] dark:bg-gray-950 hover:bg-[#fff]/70 text-primary h-8"' : ""} h-8`}
          onClick={() => {
            setTabType("api");
            onTabChange("api");
            ...
            buttonTipContent={
              <div>
                ...
                <p className="mt-4 text-sm">
                  {t("skills.supportVersions", { ns: "bs" })}
                </p>
              </div>
            }
            dropDown={
              <div className=" overflow-y-auto max-h-96 max-h">
                <RadioGroup
                  value={version.id + ""}
                  onValueChange={(vid) => {
                    if (isOnlineVersionFun() && hasChanged)
                      return setModelVersionId(vid);
                ...
                >
                  {versions.map((vers, index) => (
                    <div
                      key={vers.id}
                      className="group flex items-center gap-4 px-4 py-2 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 border-b"
                    >
                      ...
                      <div className="w-[198px]">
                        <TextInput
                          ...
                          maxLength={30}
                          onSave={(val) =>
                            changeName(vers.id, val || vers.name)
                          }
                        ></TextInput>
      ...
      <Dialog open={blocker.state === "blocked"}>
        <DialogContent className="sm:max-w-[425px]" close={false}>
          <DialogHeader>
            <DialogTitle>{t("prompt")}</DialogTitle>
            <p className="bisheng-label pt-2">
              {isOnlineVersion
                ? t("onlineVersionMessage")
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => {
                ...
                blocker.reset?.();
                if (dom)
                  dom.className = dom.className.replace("report-hidden", "");
              }}
            >
              {t("cancel")}
            </Button>
...
const useNodeEvent = (flow) => {
  ...
  return async () => {
    ...
    const promises = Object.keys(nodeValidateEntitiesRef.current).map(
      async (key) => {
    ...
    const branchLines: {
      branch: string;
      nodeIds: { branch: string; nodeId: string }[];
      end: boolean;
    }[] = [];
    const nodeMap = {};
    const treeNodeIdSet = new Set();
    const startEdge = flow.edges.find(
      (node) => node.source.indexOf("start") === 0,
    );
    ...
    const findOutType = (nodeId) => {
      ...
      return (
        flow.nodes.find((node) => node.id === nodeId)?.data.group_params[0]
          .params[1].value.type || ""
      );
    };
    ...
    traverseTree(startNodeId, "0", [
      { branch: "0", nodeId: startNodeId, type: "" },
    ]);

    // 节点连线完整校验
    if (
      treeNodeIdSet.size !==
      flow.nodes.filter((node) => node.type === "flowNode").length - 1
    ) {
      const ids = flow.nodes.reduce((res, node) => {
        if (
          node.type === "flowNode" &&
          node.data.type !== "start" &&
          !treeNodeIdSet.has(node.id)
        )
          res.push(node.id);
        return res;
      }, []);
      ...
      ([inputNodeLs, outputNodeLs, branchNodeLs], line) => {
        line.nodeIds.forEach((node) => {
          if (node.nodeId.startsWith("input")) {
            const inputNode = flow.nodes.find(
              (_node) =>
                _node.id === node.nodeId && _node.data.tab.value === "input",
            );
            // It is an input & ouput node and is different from the branch path in ids;
            if (
              inputNode &&
              !inputNodeLs.some((el) => el.branch === inputNode.branch)
            ) {
              !nodeLMap[node.branch] && inputNodeLs.push(node);
            }
          } else if (
            node.nodeId.startsWith("output") &&
            node.type === "input"
          ) {
            !nodeLMap[node.branch] && inputNodeLs.push(node);
          } else if (
            (node.nodeId.startsWith("output") && node.type === "choose") ||
            node.nodeId.startsWith("condition")
          ) {
...
const useVersion = (flow) => {
  ...
  const refrenshVersions = () => {
    return getFlowVersions(flow.id).then(({ data, total }) => {
      ...
      const currentV = data.find((el) =>
        currentvId ? el.id === currentvId : el.is_current === 1,
      );
      setVersion(currentV);
      // 记录上线的版本
  ...
  const handleChangName = (id, name) => {
    captureAndAlertRequestErrorHoc(
      updateVersion(id, { name, description: "", data: null }),
    );
    // 乐观更新
    setVersions(
  ...
  const handleDeleteVersion = (version, index) => {
    bsConfirm({
      ...
      onOk: (next) => {
        captureAndAlertRequestErrorHoc(deleteVersion(version.id)).then(
          (res) => {
            if (res === null) {
              // 乐观更新
...
const useBeforeUnload = (flow, nodes, preFlow, onPreFlowChange) => {
  ...
  const hasChanged = useMemo(() => {
    ...
    try {
      ...
      // 比较边和节点数据
      return !(
        isEqual(edges, oldEdges) &&
        nodes.every((node, i) => isEqual(node.data, oldNodes[i].data))
      );
    } catch {
