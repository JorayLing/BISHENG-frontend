...
export default function Assistants() {
  ...
  const {
    ...
    refreshData,
    filterData,
  } = useTable<AssistantItemDB>({ pageSize: 15 }, (param) =>
  ...
  return (
    <div className="h-full relative bg-background-main border-t">
      <div className="px-10 py-10 h-full overflow-y-scroll scrollbar-hide relative">
        <div className="flex space-x-4">
          <SearchInput
            className="w-64"
            placeholder={t("build.searchAssistant")}
          ...
          <div className="mt-6 flex gap-2 flex-wrap pb-20 min-w-[980px]">
            ...
            {dataSource.map((item: any, i) => (
              <CardComponent<AssistantItemDB>
                ...
                currentUser={user}
                onClick={() =>
                  item.status !== 1 && navigate("/assistant/" + item.id)
                }
                onSwitchClick={() =>
                  !item.write &&
                  item.status !== 1 &&
                  message({
                    title: t("prompt"),
                    description: t("skills.contactAdmin"),
                    variant: "warning",
                  })
                }
...
export const useQueryLabels = (t) => {
  ...
  const [filteredOptions, allOptions] = useMemo(() => {
    ...
    // 检索
    const _newOptions = options.filter(
      (op) =>
        op.label.toUpperCase().includes(searchKey.toUpperCase()) ||
        op.value === selectLabel.value,
    );
