...
export default function editAssistant() {
  ...
  const handleOnline = async (online) => {
    ...
    if (online) {
      await handleSave();
      await captureAndAlertRequestErrorHoc(
        changeAssistantStatusApi(assistantState.id, 2),
      ).then((res) => {
        if (res === false) return;
        message({
          title: t("prompt"),
      ...
      captureAndAlertRequestErrorHoc(
        changeAssistantStatusApi(assistantState.id, 1),
      ).then((res) => {
        if (res === false) return;
        changeStatus(1);
      });
    ...
    <div className="bg-background-main">
      ...
      <div className="h-[calc(100vh-70px)]">
        <div className={`flex h-full ${showApiPage ? "hidden" : ""}`}>
          <div className="w-[60%]">
            <div className="text-md font-medium leading-none p-4 shadow-sm">
              {t("build.assistantConfiguration")}
            </div>
          ...
          <div className="w-[40%] h-full bg-[#fff] dark:bg-background-main relative">
            {openChat && (
              <TestChat
                ...
                assisId={assisId}
                onClear={() => handleStartChat(false)}
              ></TestChat>
            )}
            {/* 变更触发保存的蒙版按钮 */}
            {changed && (
              <div
                className="absolute w-full bottom-0 h-60"
      ...
      <Dialog open={blocker.state === "blocked"}>
        <DialogContent className="sm:max-w-[425px]" close={false}>
          <DialogHeader>
            ...
            <DialogDescription>
              {assistantState.status === OnlineState.OnLine
                ? "助手已上线,不可进行更改"
                : "您有未保存的更改,确定要离开吗?"}
            </DialogDescription>
          </DialogHeader>
