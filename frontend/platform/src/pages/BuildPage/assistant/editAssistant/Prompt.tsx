...
export default function Prompt() {
  ...
  return (
    <div className="w-[50%] h-full bg-background-login shadow-sm p-4 overflow-y-auto scrollbar-hide">
      <div className="flex-between-center">
        <span className="text-sm font-medium leading-none">
          {t("build.assistantPortrait")}
        </span>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button variant="link" className="p-0">
              <Settings2 className="mr-1 h-4 w-4" />
              {t("build.automaticOptimization")}
            </Button>
          </DialogTrigger>
      ...
      <Textarea
        ...
        onInput={(e) =>
          dispatchAssistant("setPrompt", { prompt: e.target.value })
        }
      ></Textarea>
    </div>
  );
