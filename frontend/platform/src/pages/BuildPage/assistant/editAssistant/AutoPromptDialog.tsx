...
export default function AutoPromptDialog({ onOpenChange }) {
  ...
  const init = async () => {
    ...
    eventSource.onmessage = (event) => {
      ...
      switch (parsedData.type) {
        case "prompt":
          areaRef.current.value += parsedData.message.replace(
            "```markdown",
            "",
          );
          break;
  ...
  const handleUseAll = () => {
    ...
    dispatchAssistant("setGuideword", {
      guide_word: guideAreaRef.current.value,
    });
    dispatchAssistant("setTools", { tool_list: tools });
    dispatchAssistant("setFlows", { flow_list: flows });
    dispatchAssistant("setQuestion", { guide_question: [...question, ""] });
  ...
  return (
    <DialogContent className="sm:max-w-[925px] bg-background-login">
      <div className="flex">
        {/* 提示词 */}
        <div className="w-[50%] relative pr-6">
          <div className="flex items-center justify-between">
            <span className="text-lg font-semibold leading-none tracking-tight flex">
              {t("build.portraitOptimization")}
              {LoadType.Prompt === loading && (
                <LoadIcon className="ml-2 text-gray-600" />
              )}
            </span>
            <Button
              variant="link"
        ...
        <div className="w-[50%] border-l pl-6">
          ...
          <div className="max-h-[660px] overflow-y-auto">
            ...
            <div className="group relative pb-12 bg-gray-100 dark:bg-[#2A2B2E] mt-4 px-4 py-2 rounded-md">
              <div className="text-md mb-2 font-medium leading-none flex">
                {t("build.openingRemarks")}
                {LoadType.GuideWord === loading && (
                  <LoadIcon className="ml-2 text-gray-600" />
                )}
              </div>
              <Textarea
                ref={guideAreaRef}
                className="bg-transparent border-none bg-gray-50 dark:bg-[#171717]"
              ></Textarea>
              <Button
                className="group-hover:flex hidden h-6 absolute bottom-4 right-4"
            ...
            <div className="group relative pb-12 bg-gray-100 dark:bg-[#2A2B2E] mt-4 px-4 py-2 rounded-md">
              <div className="text-md mb-2 font-medium leading-none flex">
                ...
                {LoadType.GuideQuestion === loading && (
                  <LoadIcon className="ml-2 text-gray-600" />
                )}
              </div>
              {question.map((qs) => (
                <p
                  key={qs}
                  className="text-sm text-muted-foreground bg-gray-50 dark:bg-[#171717] px-2 py-1 rounded-xl mb-2"
                >
                  {qs}
                </p>
              ))}
