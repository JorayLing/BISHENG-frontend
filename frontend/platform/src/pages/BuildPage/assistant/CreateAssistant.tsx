  ...
  const handleSubmit = async (e) => {
    ...
    if (isValid) {
      ...
      const res = await captureAndAlertRequestErrorHoc(
        createAssistantsApi(formData.name, formData.roleAndTasks, formData.url),
      );
      if (res) {
        //@ts-ignore
      ...
      <div className="flex flex-col gap-8 py-6">
        ...
        <div className="">
          ...
          <Input
            ...
            maxLength={50}
            showCount
            placeholder={t("build.giveAssistantName")}
            className="mt-2"
            value={formData.name}
        ...
        <div className="">
          ...
          />
          {errors.roleAndTasks && (
            <p className="bisheng-tip mt-1">{errors.roleAndTasks}</p>
          )}
        </div>
      </div>
      <DialogFooter>
        <DialogClose>
          <Button
            ...
            className="px-11"
            type="button"
            onClick={() => setFormData({ name: "", roleAndTasks: "" })}
          >
            {t("cancle")}
          </Button>
        </DialogClose>
