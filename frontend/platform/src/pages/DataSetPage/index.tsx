...
export default function index() {
  ...
  const handleDownloadFile = async (name, url) => {
    ...
    if (!res.url) {
      return toast({
        variant: "error",
        description: t("dataset.fileNotFound"),
      });
    }
              ...
              <TableRow key={el.id}>
                ...
                >
                  <Button
                    variant="link"
                    className="px-1"
                    onClick={() => handleDownloadFile(el.name, el.url)}
                  >
                    {t("dataset.download")}
                  </Button>
                  <Button
                    ...
                    className="ml-4 text-red-500 px-0"
                  >
                    {t("delete")}
                  </Button>
                </TableCell>
