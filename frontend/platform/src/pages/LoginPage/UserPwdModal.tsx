...
const UserPwdModal = forwardRef<UserPwdModalRef, UserPwdModalProps>(
  (props, ref) => {
    ...
    const handleSubmit = async () => {
      ...
      if (!PWD_RULE.test(passwordRef.current.value))
        errors.push(t("login.passwordError"));
      ...
      const res = await captureAndAlertRequestErrorHoc(
        resetPasswordApi(userIdRef.current, cryptPwd),
      );
      if (res === null) {
        message({
          title: `${t("prompt")}`,
      ...
      <Dialog open={editShow} onOpenChange={setEditShow}>
        <DialogContent className="sm:max-w-[625px]">
          ...
          <DialogFooter>
            ...
            <Button type="button" className="px-11" onClick={handleSubmit}>
              {t("confirmButton")}
            </Button>
          </DialogFooter>
        </DialogContent>
