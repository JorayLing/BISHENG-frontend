...
export default function RunTest({ nodeId }) {
  ...
  const versionColWidth = useMemo(() => {
    const count =
      mulitVersionFlow.reduce((count, cur) => {
  ...
  const handleDownExcle = () => {
    ...
    const data = [
      ["测试用例", ..._mulitVersionFlow.map((version) => version.name)],
    ];
  ...
  const notDiffVersion = useMemo(
    () => !mulitVersionFlow.some((version) => version),
    [mulitVersionFlow],
  );
      ...
      <div className="bg-[#fff] dark:bg-gray-950 p-2">
        <div className="flex items-center justify-between ">
          <div className="flex gap-2 items-center">
            <Button
              size="sm"
              disabled={["all", "row", "col"].includes(runningType)}
          ...
          {isForm ? (
            <Dialog open={formShow} onOpenChange={setFormShow}>
              <DialogTrigger asChild>
                <Button
                  size="sm"
                  disabled={runningType === "all" || notDiffVersion}
                >
                  <Play size={14} />
                  {t("test.testRun")}
                </Button>
              ...
              <RunForm
                show={formShow}
                flow={mulitVersionFlow[0]}
                onChangeShow={setFormShow}
        ...
        <Table className="table-fixed">
          <TableHeader>
            <TableRow>
              <TableHead style={{ width: `${versionColWidth}%` }}>
                {t("test.testCase")}
              </TableHead>
              {mulitVersionFlow.map(
                (version) =>
                  version && (
                    <TableHead
                      key={version.id}
                      style={{ width: `${versionColWidth + 10}%` }}
                    >
              ...
              <TableHead
                className="text-right min-w-[135px]"
                style={{ width: 135 }}
              >
                <Button
                  variant="link"
                  disabled={runningType !== "" || !running}
                  onClick={handleDownExcle}
                >
                  <Download className="mr-1" size={18} />
                  {t("test.downloadResults")}
                </Button>
              </TableHead>
            </TableRow>
          </TableHeader>
            ...
            {questions.map((question, index) => (
              <TableRow>
                <TableCell>
                  <div className="flex items-center gap-2 font-medium">
                    ...
                    {question.ready && (
                      <Button
                        disabled={
                          ["all"].includes(runningType) || notDiffVersion
                        }
                        size="icon"
                ...
                {mulitVersionFlow.map(
                  (flow) =>
                    flow && (
                      <TableCell key={index + "-" + flow.id} className="">
