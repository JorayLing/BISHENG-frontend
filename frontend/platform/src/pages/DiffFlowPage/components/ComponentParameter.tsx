...
export default function ComponentParameter({
  ...
  const handleOnNewValue = (newValue: any, name) => {
    ...
    if (["index_name", "collection_name"].includes(name))
      delete node.data.node.template[name].collection_id;
    onChange(); // 更新通知
  };
  ...
  const getStrComp = (template, n) => {
    return template[n].list ? (
      ...
      <InputComponent
        editNode={true}
        disabled={_disabled}
        password={template[n].password ?? false}
        value={template[n].value ?? ""}
        onChange={(t) => {
          handleOnNewValue(t, n);
        }}
    ...
    <>
      {keys.map((n, i) => {
          ...
          if (template[n].options) {
            return children(
              ...
              <Dropdown
                ...
                value={template[n].value ?? "Choose an option"}
              ></Dropdown>,
            );
          } else {
        ...
        switch (template[n].type) {
          ...
          default:
            return children(n, name, <div className="hidden"></div>);
        }
      })}
    </>
  );
