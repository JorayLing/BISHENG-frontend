...
export default function Component({
  ...
  if (!version)
    return (
      <div className="bg-[#fff] dark:bg-gray-950 rounded-md p-2 shadow-sm">
            ...
            <SelectContent>
              {options.map((vs) => (
                <SelectItem
                  ...
                  value={vs.id}
                  textValue={"vs.name"}
                  disabled={disables.includes(vs.id)}
                >
  ...
  return (
    <div
      className={
        "bg-[#fff] dark:bg-gray-950 rounded-md p-2 shadow-sm " + className
      }
    >
        ...
        <Select value={version.id} onValueChange={onChangeVersion}>
          ...
          <SelectContent>
            {options.map((vs) => (
              ...
              >
                <div className="flex justify-between w-64">
                  <span className="w-36 overflow-hidden text-ellipsis whitespace-nowrap text-left">
                    {vs.name}
                  </span>
                  <span className="text-xs text-muted-foreground">
                    {vs.update_time.replace("T", " ").substring(0, 16)}
      ...
      <div className="max-h-52 overflow-y-auto pb-10">
        ...
        {nodes.map((node) => (
          <div className="flex odd:bg-gray-50 bg-[#f4f5f8] dark:bg-[#212121] gap-1 mt-1 px-2 py-1 text-sm rounded-sm">
            <span className="min-w-12 w-28 break-all self-center">
              {node.data.type}
            </span>
