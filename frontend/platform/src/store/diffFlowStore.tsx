...
export const useDiffFlowStore = create<State & Actions>((set, get) => ({
  ...
  removeVersionFlow(index) {
    set((state) => ({
      mulitVersionFlow: state.mulitVersionFlow.filter((_, i) => i !== index),
    }));
  },
  ...
  addQuestion(q) {
    set((state) => ({
      questions: [
        ...state.questions,
        { q, id: generateUUID(5), ready: get().running },
      ],
    }));
  },
  updateQuestion(q, index) {
    set((state) => ({
      questions: state.questions.map((el, i) =>
        i === index ? { ...el, q, ready: get().running } : el,
      ),
    }));
  },
...
const runTest = ({
  ...
  return new Promise((resolve, reject) => {
    const apiUrl = `${__APP_ENV__.BASE_URL}/hjapi/v1/flows/compare/stream?data=${encodeURIComponent(data)}`;
    ...
    eventSource.onmessage = (event) => {
      ...
      if (!type) {
        refs[`${questionIndexs[question_index]}-${version_id}`].current.setData(
          answer,
        );
      } else if (type === "end") {
        resolve("");
