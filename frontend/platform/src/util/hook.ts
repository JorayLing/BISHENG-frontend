...
export function useHasForm(flow) {
  return useMemo(() => {
    ...
    return !!flow?.data?.nodes.find((node) =>
  ...
  const loadData = () => {
    ...
    apiFun({ ...page, ...paramRef.current })
      .then((res) => {
        ...
        if (!("total" in res))
          return console.error("该接口不支持分页，无法正常使用 useTable");
        setData(res.data);
        setTotal(res.total);
        setLoading(false);
      })
...
export function useUndoRedo<T>(maxHistorySize = 100) {
  ...
  const takeSnapshot = useCallback(() => {
    setPast((old) => {
      const newPast = old.slice(old.length - maxHistorySize + 1, old.length);
      newPast.push({
