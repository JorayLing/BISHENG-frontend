...
export const flowVersionCompatible = (flow) => {
  flow.nodes.forEach((node) => {
    switch (node.data.type) {
      case "start":
        comptibleStart(node.data);
        break;
      case "input":
        comptibleInput(node.data);
        break;
      case "agent":
        comptibleAgent(node.data);
        break;
      case "output":
        comptibleOutput(node.data);
        break;
      case "llm":
        comptibleLLM(node.data);
        break;
    }
  });
  return flow;
...
const comptibleStart = (node) => {
  if (!node.v) {
    ...
    node.group_params[1].params[2].value =
      node.group_params[1].params[2].value.map((item) => ({
        key: generateUUID(6),
        value: item,
      }));
    // TODO 历史使用过的预知问题变量替换
...
const comptibleInput = (node) => {
  ...
  if (!node.v) {
    ...
    let i = 0;
    node.group_params[0].params[1].value =
      node.group_params[0].params[1].value.map((item) => {
        if (item.type === "file") {
          i++;
          return {
            ...item,
  ...
  if (node.v == 1) {
    ...
    // 兼容文件类型
    const formInput = node.group_params[0].params.find(
      (item) => item.key === "form_input",
    );
