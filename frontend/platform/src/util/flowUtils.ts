      ...
      if (param.type === "var_textarea" && typeof param.value === "string") {
        ...
        param.value = param.value.replace(
          ...
          (match, prefix = "", expression) => {
            if (param.varZh) {
              param.varZh[`${prefix}${id}.${expression}`] =
                `${prefix}${expression}`;
            } else {
              param.varZh = {
...
export function isVarInFlow(nodeId, nodes, varName, varNameCn) {
    ...
    varNodeId === node.id
      ? node.data.group_params.some((group) =>
          group.params.some((param) => {
            if (
              param.type === "input_list" &&
              varName.indexOf("preset_question") !== -1
            ) {
              const questionId = varName.split("#")[1];
              const quwstionStr = varNameCn?.split("/")[1] || "";
              return param.value.some(
                (item) => item.key === questionId && item.value === quwstionStr,
              ); // id and name 必须一致
            } else if (
              (param.type === "var" &&
                Array.isArray(param.value) &&
                param.value.length) ||
              param.type === "code_output"
            ) {
              return param.value.some(
                (item) => `${node.id}.${item.key}` === varName,
              );
              ...
              return param.value.some((item) => {
                ...
                const vars = [
                  `${node.id}.${item.key}`,
                  `${node.id}.${item.file_content}`,
                  `${node.id}.${item.file_path}`,
                ];
                // 图片类型追加校验变量
                item.file_type !== "file" &&
                  vars.push(`${node.id}.${item.image_file}`);
  ...
  return res
    ? ""
    : i18next.t("nodeErrorMessage", { ns: "flow", nodeName, varNameCn });
}
...
export function findParallelNodes(a, b) {
  ...
  for (let i = 0; i < a.length; i++) {
    ...
    for (let j = i + 1; j < a.length; j++) {
      ...
      if (isSameParent) {
        const isUnderBBranch = parents.some(
          (node) => node.branch === parentBranch1,
        );
        if (isUnderBBranch) {
          continue;
        }
